  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <StatCard label="Total Tokens" value={formatNumber(
          (overview?.totalInputTokens ?? 0) + (overview?.totalOutputTokens ?? 0)
        )} sub={`${formatNumber(overview?.totalInputTokens ?? 0)} in / ${formatNumber(overview?.totalOutputTokens ?? 0)} out`} />
        <StatCard label="Sessions" value={String(overview?.totalSessions ?? 0)} sub={`${overview?.totalTurns ?? 0} turns`} />
        <StatCard label="Est. Cost" value={`$${(overview?.estimatedCostUsd ?? 0).toFixed(2)}`} />
        <StatCard label="Active Users" value={String(overview?.activeUsers ?? 0)} />
      </div>
        ...
        <div className="overflow-hidden rounded-lg border border-white/5">
          <table className="w-full text-xs">
            ...
            <tbody className="text-text-secondary">
              <tr className="border-b border-white/5"><td className="px-3 py-1.5">Haiku</td><td className="px-3 py-1.5 text-right">$0.25</td><td className="px-3 py-1.5 text-right">$1.25</td></tr>
              <tr className="border-b border-white/5"><td className="px-3 py-1.5">Sonnet</td><td className="px-3 py-1.5 text-right">$3.00</td><td className="px-3 py-1.5 text-right">$15.00</td></tr>
