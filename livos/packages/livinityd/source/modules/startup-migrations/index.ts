...
import {findExternalLivinityInstall, runPreMigrationChecks, migrateData} from '../migration/migration.js'
...
class Migration {
	...
	async migrateLegacyLinuxData() {
		...
		if (await this.livinityd.user.exists()) return
		...
		const currentInstall = this.livinityd.dataDirectory
		await runPreMigrationChecks(currentInstall, externalLivinityInstall as string, this.livinityd, false)
		await this.livinityd.server.start()
		await migrateData(currentInstall, externalLivinityInstall as string, this.livinityd)
		this.logger.log('Migration complete!')
	}

	async activateImportedDataDirectory() {
		const importData = `${this.livinityd.dataDirectory}/import`
		const importDataExists = await fse.exists(importData)
		if (!importDataExists) return
		this.logger.log('Found Livinity data to import, activating...')
		...
		// This is fine to do on Livinity Home because all of /home is on the large data partition.
		...
		const temporaryData = `${this.livinityd.dataDirectory}-import-temp`
		await fse.move(importData, temporaryData, {overwrite: true})
		await fse.move(temporaryData, this.livinityd.dataDirectory, {overwrite: true})
	}

	async migrateLegacyData() {
		// Check for a legacy <1.0 Livinity data directory
		const userJsonPath = `${this.livinityd.dataDirectory}/db/user.json`
		const userJsonExists = await fse.exists(userJsonPath)
		if (!userJsonExists) return
		this.logger.log('Found legacy Livinity data, migrating...')
		...
		// Migrate data
		await this.livinityd.user.setName(legacyData.name)
		await this.livinityd.user.setHashedPassword(legacyData.password)
		...
		const envPath = `${this.livinityd.dataDirectory}/.env`
	...
	async start() {
		...
		await fse.ensureDir(this.livinityd.dataDirectory)

		// Check for a data directory to import
		try {
			await this.activateImportedDataDirectory()
		} catch (error) {
			this.logger.error(`Failed to activate imported Livinity data`, error)
		}

		// Check for a legacy <1.0 Livinity data directory and migrate to 1.0 format if found
		try {
		...
		const previousVersion = await this.livinityd.store.get('version')
		await this.livinityd.store.set('version', this.livinityd.version)

		// Add notification if version changed
		if (previousVersion && previousVersion !== this.livinityd.version) {
			await this.livinityd.notifications.add('livos-updated').catch(() => {})
		}
