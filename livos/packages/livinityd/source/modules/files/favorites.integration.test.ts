...
describe('favorites()', () => {
	test('throws invalid error without auth token', async () => {
		await expect(livinityd.unauthenticatedClient.files.favorites.query()).rejects.toThrow('Invalid token')
	})

	test('returns default favorites on first start', async () => {
		const favorites = await livinityd.client.files.favorites.query()
	...
	test('only returns existing directories', async () => {
		...
		const testDirectory1 = `${livinityd.instance.dataDirectory}/home/favorites-existing-test1`
		const testDirectory2 = `${livinityd.instance.dataDirectory}/home/favorites-existing-test2`
		await fse.mkdir(testDirectory1)
		await fse.mkdir(testDirectory2)

		// Add both directories to favorites
		await livinityd.client.files.addFavorite.mutate({
...
describe('#handleFileChange()', () => {
	test('automatically removes favorites when directory is deleted', async () => {
		// Create test directories
		const testDirectoryToDelete = `${livinityd.instance.dataDirectory}/home/favorites-auto-remove-test`
		const testDirectoryToKeep = `${livinityd.instance.dataDirectory}/home/favorites-keep-test`
		...
		let favorites = await livinityd.client.files.favorites.query()
		...
		favorites = await livinityd.instance.store.get('files.favorites')
...
describe('addFavorite()', () => {
	test('throws invalid error without auth token', async () => {
		await expect(livinityd.unauthenticatedClient.files.addFavorite.mutate({path: '/Home/Documents'})).rejects.toThrow(
			'Invalid token',
		)
	})
	...
	test('throws on non-directory paths', async () => {
		// Create test file
		const testDirectory = `${livinityd.instance.dataDirectory}/home/favorites-test`
		await fse.mkdir(testDirectory)
		await fse.writeFile(`${testDirectory}/file.txt`, 'test content')

		// Attempt to favorite a file
		await expect(
			livinityd.client.files.addFavorite.mutate({
	...
	test('ignores duplicate favorites', async () => {
		// Create test directory
		const testDirectory = `${livinityd.instance.dataDirectory}/home/favorites-duplicate-test`
		await fse.mkdir(testDirectory)
...
describe('removeFavorite()', () => {
	test('successfully removes a directory from favorites', async () => {
		// Create test directory
		const testDirectory = `${livinityd.instance.dataDirectory}/home/favorites-remove-test`
		await fse.mkdir(testDirectory)
		...
		await expect(
			livinityd.client.files.removeFavorite.mutate({
				path: '/Home/favorites-remove-test',
			}),
		).resolves.toBe(true)
