...
// Create new livinityd instance for each test to clear recent state
beforeEach(async () => {
...
test('recents() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.files.recents.query()).rejects.toThrow('Invalid token')
})

test('recents() returns recently modified files in correct order', async () => {
	// Create test directory and files
	const testDirectory = `${livinityd.instance.dataDirectory}/home/recents-test`
...
test('recents() updates when files are modified', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/recents-update-test`
	await fse.mkdir(testDirectory)
	...
	let recentFiles = await livinityd.client.files.recents.query()
	...
	recentFiles = await livinityd.client.files.recents.query()
	expect(recentFiles.map((file) => file.name)).toStrictEqual(['original.txt'])
})

test('recents() removes deleted files', async () => {
	// Create test directory and files
	const testDirectory = `${livinityd.instance.dataDirectory}/home/recents-delete-test`
	await fse.mkdir(testDirectory)
...
test('recents() ignores hidden files', async () => {
	// Create test directory and files
	const testDirectory = `${livinityd.instance.dataDirectory}/home/recents-hidden-test`
	await fse.mkdir(testDirectory)
...
test('recents() ignores files after they are sent to trash', async () => {
	// Create test directory and files
	const testDirectory = `${livinityd.instance.dataDirectory}/home/recents-trash-test`
	await fse.mkdir(testDirectory)
	...
	await livinityd.client.files.trash.mutate({path: virtualPath})
...
test('recents() persists after livinityd restart', async () => {
	// Create test directory and file
	const testDirectory = `${livinityd.instance.dataDirectory}/home/recents-persist-test`
	await fse.mkdir(testDirectory)
	...
	let recentFileInStore = await livinityd.instance.store.get('files.recents')
	...
	recentFileInStore = await livinityd.instance.store.get('files.recents')
...
test('recents() ignores app data files', async () => {
	...
	const homeDirectory = `${livinityd.instance.dataDirectory}/home/recents-app-test`
	const appDirectory = `${livinityd.instance.dataDirectory}/app-data/recents-app-test`
...
test('recents() respects maximum number of entries', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/recents-max-test`
	await fse.mkdir(testDirectory)
