...
// Setup livinityd once for all tests
beforeEach(async () => {
...
describe('enabled', () => {
	test('is enabled on Livinity Home and amd64', async () => {
		await expect(livinityd.instance.files.externalStorage.supported()).resolves.toBe(true)
	})

	test('is disabled on Raspberry Pi', async () => {
		isRaspberryPiMockValue = true
		await expect(livinityd.instance.files.externalStorage.supported()).resolves.toBe(false)
	})
})
...
describe('file permissions', () => {
	test('allows hard deletion of external files', async () => {
		...
		const testFile = `${livinityd.instance.dataDirectory}/external/My Portable SSD/file.txt`
		await fse.ensureFile(testFile)

		// Attempt to delete the directory
		await expect(
			livinityd.client.files.delete.mutate({path: '/External/My Portable SSD/file.txt'}),
	...
	test('does not allow soft trash of external files', async () => {
		...
		// Attempt to delete the directory
		await expect(livinityd.client.files.trash.mutate({path: '/External/My Portable SSD/file.txt'})).rejects.toThrow(
	...
	test('mount points are protected paths', async () => {
		...
		// Trash
		await expect(livinityd.client.files.trash.mutate({path: '/External/My Portable SSD'})).rejects.toThrow(
			'[operation-not-allowed]',
		)
		// Delete
		await expect(livinityd.client.files.delete.mutate({path: '/External/My Portable SSD'})).rejects.toThrow(
			'[operation-not-allowed]',
		)
		// Move
		await expect(
			livinityd.client.files.move.mutate({path: '/External/My Portable SSD', toDirectory: '/Home'}),
		).rejects.toThrow('[operation-not-allowed]')
		// Rename
		await expect(
			livinityd.client.files.rename.mutate({path: '/External/My Portable SSD', newName: 'My Renamed SSD'}),
		).rejects.toThrow('[operation-not-allowed]')
	})
})
...
describe('files.mountedExternalDevices', () => {
	test('throws unauthorized error without auth token', async () => {
		await expect(livinityd.unauthenticatedClient.files.externalDevices.query()).rejects.toThrow('Invalid token')
	})
	...
	test('returns empty array when no external devices are attached', async () => {
		...
		const externalDevices = await livinityd.client.files.externalDevices.query()
	...
	test('returns mounted external devices', async () => {
		// Mock lsblk command to return a valid response for a mounted external disk
		mockCommand = (command: string) => {
			// We need to replace any /home/livinity/livinity paths with the current instances
			...
			if (command.startsWith('lsblk'))
				return JSON.stringify(LSBLK_EXTERNAL_DISK_MOUNTED).replaceAll(
					'/home/livinity/livinity',
					livinityd.instance.dataDirectory,
				)
		}
...
describe('files.unmountExternalDevice', () => {
	test('throws unauthorized error without auth token', async () => {
		await expect(livinityd.unauthenticatedClient.files.unmountExternalDevice.mutate({deviceId: 'sda'})).rejects.toThrow(
			'Invalid token',
		)
	})
	...
	test('throws error on invalid device id', async () => {
		...
		// Missing device
		await expect(livinityd.client.files.unmountExternalDevice.mutate({deviceId: 'sdz'})).rejects.toThrow(
			'[invalid-device-id]',
		)

		// Passed partition id instead of device id
		await expect(livinityd.client.files.unmountExternalDevice.mutate({deviceId: 'sda2'})).rejects.toThrow(
			'[invalid-device-id]',
		)
	})
	...
	test('unmounts external device', async () => {
		...
		// Create mountpoint
		const mountPoint = `${livinityd.instance.dataDirectory}/external/Red T5`
		...
		await expect(livinityd.client.files.unmountExternalDevice.mutate({deviceId: 'sda'})).resolves.toBe(true)
...
describe('files.isExternalDeviceConnectedOnUnsupportedDevice', () => {
	test('throws unauthorized error without auth token', async () => {
		await expect(
			livinityd.unauthenticatedClient.files.isExternalDeviceConnectedOnUnsupportedDevice.query(),
		).rejects.toThrow('Invalid token')
	})

	test('returns false when on Livinity Home and amd64 but no external devices connected', async () => {
		...
		// Check the result
		const result = await livinityd.client.files.isExternalDeviceConnectedOnUnsupportedDevice.query()
		expect(result).toBe(false)
	})

	test('returns false when on Livinity Home and amd64 and external devices connected', async () => {
		// Mock lsblk to show an external USB device
		mockCommand = (command: string) => {
			if (command.startsWith('lsblk')) return JSON.stringify(LSBLK_EXTERNAL_DISK_ATTACHED)
			if (command.startsWith('df')) return {stdout: 'Filesystem\n/dev/nvme0n1p4'}
...
describe('externalstorage.#cleanLeftOverMountPoints()', () => {
	test('cleans up any left over mount points on startup', async () => {
		...
		const leftoverMountPoint = `${livinityd.instance.dataDirectory}/external/My Portable SSD`
...
describe('externalstorage.#mountExternalDevices', () => {
	test('does nothing when no external devices are attached', async () => {
		...
		// Simulate disk event
		await livinityd.instance.eventBus.emit('system:disk:change')
...
describe('filesystem permissions', () => {
	test('/External cannot have new user created directories', async () => {
		await expect(livinityd.client.files.createDirectory.mutate({path: '/External/test'})).rejects.toThrow(
			'[operation-not-allowed]',
		)
	})

	test('/External cannot have directories copied directly into it', async () => {
		await expect(livinityd.client.files.copy.mutate({path: '/Home/Documents', toDirectory: '/External'})).rejects.toThrow(
			'[operation-not-allowed]',
		)
	})

	test('/External cannot have directories moved directly into it', async () => {
		await expect(livinityd.client.files.move.mutate({path: '/Home/Documents', toDirectory: '/External'})).rejects.toThrow(
			'[operation-not-allowed]',
		)
	})
})
...
// Taken from a physical Livinity Home with:
			...
			children: [
				...
				},
				{
					...
					fsroots: [
						'/livinity-os/var/log',
						'/livinity-os/var/log',
						'/livinity-os/var/lib/systemd/timesync',
						'/livinity-os/var/lib/systemd/timesync',
						'/livinity-os/var/lib/docker',
						'/livinity-os/var/lib/docker',
						'/livinity-os/home',
						'/livinity-os/home',
						'/',
	...
	blockdevices: [
		{
			...
			children: [
				...
				},
				{
					...
					state: null,
					subsystems: 'block:scsi:usb:pci',
					mountpoint: '/mnt/root/home/livinity/livinity/external/Red T5',
					mountpoints: [
						'/mnt/root/home/livinity/livinity/external/Red T5',
						'/mnt/root/data/livinity-os/home/livinity/livinity/external/Red T5',
						'/data/livinity-os/home/livinity/livinity/external/Red T5',
						'/home/livinity/livinity/external/Red T5',
					],
					tran: null,
					type: 'part',
					uuid: '67F5-306E',
