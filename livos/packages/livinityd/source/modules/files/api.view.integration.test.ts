...
test('GET /api/files/view throws unauthorized error without cookie', async () => {
	const error = await livinityd.unauthenticatedApi.get('files/view').catch((error) => error)
...
test('GET /api/files/view throws 404 error without path parameter', async () => {
	const error = await livinityd.api.get('files/view').catch((error) => error)
	expect(error).toBeInstanceOf(Error)
...
test('GET /api/files/view throws 404 error when file does not exist', async () => {
	const error = await livinityd.api.get('files/view?path=/Home/does-not-exist').catch((error) => error)
	expect(error).toBeInstanceOf(Error)
...
test('GET /api/files/view throws 404 error on directory traversal attempt', async () => {
	const error = await livinityd.api.get('files/view?path=/Home/../../../../etc/passwd').catch((error) => error)
	expect(error).toBeInstanceOf(Error)
	expect(error.response.statusCode).toBe(404)
	expect(error.response.body).toMatchObject({error: 'not found'})
})
...
test('GET /api/files/view throws 404 error on relative path', async () => {
	...
	for (const path of paths) {
		const error = await livinityd.api.get(`files/view?path=${path}`).catch((error) => error)
		expect(error).toBeInstanceOf(Error)
		expect(error.response.statusCode).toBe(404)
		expect(error.response.body).toMatchObject({error: 'not found'})
	}
...
test('GET /api/files/view throws 404 error on symlink traversal attempt', async () => {
	...
	const error = await livinityd.api.get('files/view?path=/Home/symlink-to-root/etc/passwd').catch((error) => error)
	expect(error).toBeInstanceOf(Error)
	expect(error.response.statusCode).toBe(404)
	expect(error.response.body).toMatchObject({error: 'not found'})
...
test('GET /api/files/view throws 404 error when trying to view a directory', async () => {
	...
	const error = await livinityd.api.get('files/view?path=/Home/test-dir').catch((error) => error)
	expect(error).toBeInstanceOf(Error)
	expect(error.response.statusCode).toBe(400)
...
test('GET /api/files/view serves a file with a valid cookie', async () => {
	...
	// View the file
	const response = await livinityd.api.get('files/view?path=/Home/file.txt', {
		responseType: 'text',
	})
...
test('GET /api/files/view handles files with special characters in name', async () => {
	...
	// View the file
	const response = await livinityd.api.get(`files/view?path=/Home/${encodeURIComponent(fileName)}`, {
		responseType: 'text',
	})
...
test('GET /api/files/view handles files with URL-encoded characters in path', async () => {
	...
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/${filename}`, 'url encoded content')
...
test('GET /api/files/view handles files with zero content correctly', async () => {
	...
	// View the file
	const response = await livinityd.api.get('files/view?path=/Home/empty-file.txt', {
		responseType: 'text',
	})
...
test('GET /api/files/view handles binary files correctly', async () => {
	...
	// View the file
	const response = await livinityd.api.get('files/view?path=/Home/binary-file.bin', {
		responseType: 'buffer',
	})
