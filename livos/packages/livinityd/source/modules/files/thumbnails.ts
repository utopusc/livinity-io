...
export default class Thumbnails {
	...
	constructor(livinityd: Livinityd) {
		...
		this.thumbnailDirectory = `${livinityd.dataDirectory}/thumbnails`
	}
	...
	async start() {
		...
		this.#removeDiskChangeListener = this.#livinityd.eventBus.on('system:disk:change', () =>
	...
	// This is used by this.livinityd.files.status() to attach an existing thumbnail to a file (as an api endpoint URL)
