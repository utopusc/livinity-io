...
test('list() throws invalid error whithout auth token', async () => {
	await expect(livinityd.unauthenticatedClient.files.list.query({path: '/'})).rejects.toThrow('Invalid token')
})

test('list() throws on directory traversal attempt', async () => {
	await expect(livinityd.client.files.list.query({path: '/Home/../../../../etc'})).rejects.toThrow('[invalid-base]')
})
...
test('list() throws on symlink traversal attempt', async () => {
	...
	await $`ln -s / ${livinityd.instance.dataDirectory}/home/symlink-to-root`
	// Ensure the symlink exists at the correct location
	await expect(livinityd.client.files.list.query({path: '/Home'})).resolves.toMatchObject({
	...
	// Attempt to list it
	await expect(livinityd.client.files.list.query({path: '/Home/symlink-to-root'})).rejects.toThrow('[escapes-base]')
...
test('list() throws on relative paths', async () => {
	await Promise.all(
		['', ' ', '.', '..', 'Home', 'Home/..', 'Home/Documents'].map((path) =>
			expect(livinityd.client.files.list.query({path})).rejects.toThrow('[path-not-absolute]'),
		),
	)
})
...
test('list() throws on non existent paths', async () => {
	await Promise.all([
		expect(livinityd.client.files.list.query({path: '/DoesNotExist'})).rejects.toThrow('[invalid-base]'),
		expect(livinityd.client.files.list.query({path: '/Home/DoesNotExist'})).rejects.toThrow('[does-not-exist]'),
	])
})
...
test('list() returns correct types for various files and directories', async () => {
	...
	const mimeDir = `${livinityd.instance.dataDirectory}/home/mime-test`
	...
	// Query the directory
	const mimeTypes = await livinityd.client.files.list.query({path: '/Home/mime-test'})
...
test('list() shows dotfiles', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/dotfiles-test`
	...
	const listing = await livinityd.client.files.list.query({
...
test('list() hides .DS_Store files', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/ds-store-test`
	await fse.mkdir(testDirectory)
...
test('list() paginates directory listings', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/pagination-test`
	await fse.mkdir(testDirectory)
	...
	const firstPage = await livinityd.client.files.list.query({path: '/Home/pagination-test'})
	...
	const secondPage = await livinityd.client.files.list.query({
	...
	const thirdPage = await livinityd.client.files.list.query({
		path: '/Home/pagination-test',
...
test('list() paginates directory listings with a custom limit', async () => {
	// Create a test directory with 150 files
	const testDirectory = `${livinityd.instance.dataDirectory}/home/custom-limit-test`
	await fse.mkdir(testDirectory)
	...
	const result = await livinityd.client.files.list.query({
...
test("list() truncates a listing if it's larger than the max listing size", async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/max-listing-size`
	await fse.mkdir(testDirectory)
	await Promise.all(
	...
	await expect(livinityd.client.files.list.query({path: '/Home/max-listing-size'})).resolves.not.toHaveProperty(
		'truncatedAt',
	)
	...
	await expect(livinityd.client.files.list.query({path: '/Home/max-listing-size'})).resolves.toHaveProperty(
		'truncatedAt',
		maxListingSize,
	)
...
test('list() sorts by name', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/sort-by-name-test`
	await fse.mkdir(testDirectory)
	...
	// Test ascending sort
	const ascending = await livinityd.client.files.list.query({
	...
	const descending = await livinityd.client.files.list.query({
		path: '/Home/sort-by-name-test',
		sortBy: 'name',
...
test('list() sorts by modified time', async () => {
	// Create a test directory with files - using unique path
	const testDirectory = `${livinityd.instance.dataDirectory}/home/sort-by-modified-test`
	await fse.mkdir(testDirectory)
...
test('list() sorts by size', async () => {
	// Create a test directory with files - using unique path
	const testDirectory = `${livinityd.instance.dataDirectory}/home/sort-by-size-test`
	await fse.mkdir(testDirectory)
...
test('list() sorts by type', async () => {
	// Create a test directory with files - using unique path
	const testDirectory = `${livinityd.instance.dataDirectory}/home/sort-by-type-test`
	await fse.mkdir(testDirectory)
...
test('list() sorts files numerically by name', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/numeric-sort-test`
	await fse.mkdir(testDirectory)
...
test('list() falls back to name sorting when numeric values are equal', async () => {
	// Create a test directory with files - using unique path
	const testDirectory = `${livinityd.instance.dataDirectory}/home/sort-fallback-test`
	await fse.mkdir(testDirectory)
...
test('list() reports size as zero for directories', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/dir-size-test`
	await fse.mkdir(testDirectory)
...
test('list() reports correct size for files in bytes', async () => {
	// Create a test directory with files - using unique path
	const testDirectory = `${livinityd.instance.dataDirectory}/home/file-size-test`
	await fse.mkdir(testDirectory)
...
test('list() reports correct modified time for a single file', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/modified-time-test`
	await fse.mkdir(testDirectory)
...
test('list() handles unreadable files gracefully without killing the entire listing', async () => {
	// Create a test directory - using unique path
	const testDirectory = `${livinityd.instance.dataDirectory}/home/unreadable-files-test`
	await fse.mkdir(testDirectory)
	...
	// Query the directory listing
	await expect(
		livinityd.client.files.list.query({
