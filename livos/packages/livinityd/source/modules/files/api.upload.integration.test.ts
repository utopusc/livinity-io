...
afterAll(() => livinityd.cleanup())
beforeEach(() => fse.emptyDir(`${livinityd.instance.dataDirectory}/home`))
afterEach(() => vi.restoreAllMocks())

test('POST /api/files/upload throws unauthorized error without cookie', async () => {
	const error = await livinityd.unauthenticatedApi
...
test('POST /api/files/upload throws 400 error without path parameter', async () => {
	const error = await livinityd.api.post('files/upload', {body: 'test content'}).catch((error) => error)
	expect(error).toBeInstanceOf(Error)
...
test('POST /api/files/upload throws 400 error on directory traversal attempt', async () => {
	const error = await livinityd.api
...
test('POST /api/files/upload throws 400 error on relative path', async () => {
	...
	for (const path of paths) {
		const error = await livinityd.api.post(`files/upload?path=${path}`, {body: 'test content'}).catch((error) => error)
		expect(error).toBeInstanceOf(Error)
		expect(error.response.statusCode).toBe(400)
		expect(error.response.body).toMatchObject({error: 'invalid path'})
	}
...
test('POST /api/files/upload throws 400 error on symlink traversal attempt', async () => {
	...
	await fse.ensureSymlink('/', `${livinityd.instance.dataDirectory}/home/symlink-to-root`)
	...
	// Clean up
	await fse.remove(`${livinityd.instance.dataDirectory}/home/symlink-to-root`)
})

test('POST /api/files/upload successfully uploads a file with valid cookie and returns success response', async () => {
	// Upload a file
	const response = await livinityd.api.post('files/upload?path=/Home/new-file.txt', {
	...
	const exists = await fse.pathExists(`${livinityd.instance.dataDirectory}/home/new-file.txt`)
	expect(exists).toBe(true)

	// Verify the content
	const content = await fse.readFile(`${livinityd.instance.dataDirectory}/home/new-file.txt`, 'utf8')
...
test('POST /api/files/upload creates parent directories if they do not exist', async () => {
	...
	const response = await livinityd.api.post('files/upload?path=/Home/new-dir/sub-dir/new-file.txt', {
		body: 'nested content',
	})
	...
	const exists = await fse.pathExists(`${livinityd.instance.dataDirectory}/home/new-dir/sub-dir/new-file.txt`)
	expect(exists).toBe(true)

	// Verify the content
	const content = await fse.readFile(`${livinityd.instance.dataDirectory}/home/new-dir/sub-dir/new-file.txt`, 'utf8')
...
test('POST /api/files/upload handles files with special characters in name', async () => {
	...
	// Upload the file
	const response = await livinityd.api.post(`files/upload?path=/Home/${encodeURIComponent(fileName)}`, {
	...
	// Verify the file was created
	const exists = await fse.pathExists(`${livinityd.instance.dataDirectory}/home/${fileName}`)
	expect(exists).toBe(true)

	// Verify the content
	const content = await fse.readFile(`${livinityd.instance.dataDirectory}/home/${fileName}`, 'utf8')
...
test('POST /api/files/upload handles empty files correctly', async () => {
	...
	const response = await livinityd.api.post('files/upload?path=/Home/empty-file.txt', {body: ''})

	// Assert the response is correct
	expect(response.statusCode).toBe(200)

	// Verify the file was created
	const exists = await fse.pathExists(`${livinityd.instance.dataDirectory}/home/empty-file.txt`)
	expect(exists).toBe(true)

	// Verify the content is empty
	const content = await fse.readFile(`${livinityd.instance.dataDirectory}/home/empty-file.txt`, 'utf8')
	expect(content).toBe('')
})
...
test('POST /api/files/upload handles binary data correctly', async () => {
	...
	// Upload binary file
	const response = await livinityd.api.post('files/upload?path=/Home/binary-file.bin', {
		body: binaryData,
	})
	...
	// Verify the file was created
	const exists = await fse.pathExists(`${livinityd.instance.dataDirectory}/home/binary-file.bin`)
	expect(exists).toBe(true)

	// Verify the content
	const content = await fse.readFile(`${livinityd.instance.dataDirectory}/home/binary-file.bin`)
...
test('POST /api/files/upload handles write errors correctly', async () => {
	...
	const systemPath = `${livinityd.instance.dataDirectory}/home/should-fail.txt`
	const temporarySystemPath = `${systemPath}.livinity-upload`
...
test('POST /api/files/upload correctly handles streaming data in chunks', async () => {
	...
	const systemPath = `${livinityd.instance.dataDirectory}/home/streaming-test.txt`
	...
	const uploadStream = livinityd.api.stream.post(`files/upload?path=${filePath}`)
...
test('POST /api/files/upload cleans up temporary files when client aborts partially uploaded file', async () => {
	...
	const systemPath = `${livinityd.instance.dataDirectory}/home/aborted-upload.txt`
	const directory = nodePath.dirname(systemPath)
	const fileName = nodePath.basename(systemPath)
	const temporarySystemPath = nodePath.join(directory, `.${fileName}.livinity-upload`)
...
test('POST /api/files/upload with collision=error (default) throws 400 when file already exists', async () => {
	// Create a file
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/collision-test.txt`, 'original content')
	...
	const content = await fse.readFile(`${livinityd.instance.dataDirectory}/home/collision-test.txt`, 'utf8')
	expect(content).toBe('original content')
})

test('POST /api/files/upload with collision=error throws 400 when explicitly set and file already exists', async () => {
	// Create a file
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/explicit-error-test.txt`, 'original content')
	...
	// Verify the file wasn't changed
	const content = await fse.readFile(`${livinityd.instance.dataDirectory}/home/explicit-error-test.txt`, 'utf8')
	expect(content).toBe('original content')
})

test('POST /api/files/upload with collision=keep-both creates uniquely named file when file already exists', async () => {
	// Create a file
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/keep-both-test.txt`, 'original content')

	// Upload to the same path with keep-both strategy
	const response = await livinityd.api.post('files/upload?path=/Home/keep-both-test.txt&collision=keep-both', {
		body: 'new content',
	})
	...
	const originalContent = await fse.readFile(`${livinityd.instance.dataDirectory}/home/keep-both-test.txt`, 'utf8')
	expect(originalContent).toBe('original content')

	const newContent = await fse.readFile(`${livinityd.instance.dataDirectory}/home/keep-both-test (2).txt`, 'utf8')
...
test('POST /api/files/upload with collision=keep-both increments number for multiple collisions', async () => {
	...
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/multiple-test.txt`, 'original content')
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/multiple-test (2).txt`, 'first duplicate')

	// Upload to the same path with keep-both strategy
	const response = await livinityd.api.post('files/upload?path=/Home/multiple-test.txt&collision=keep-both', {
	...
	const originalContent = await fse.readFile(`${livinityd.instance.dataDirectory}/home/multiple-test.txt`, 'utf8')
	expect(originalContent).toBe('original content')

	const firstDuplicate = await fse.readFile(`${livinityd.instance.dataDirectory}/home/multiple-test (2).txt`, 'utf8')
	expect(firstDuplicate).toBe('first duplicate')

	const secondDuplicate = await fse.readFile(`${livinityd.instance.dataDirectory}/home/multiple-test (3).txt`, 'utf8')
...
test('POST /api/files/upload with collision=replace overwrites existing file', async () => {
	// Create a file
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/replace-test.txt`, 'original content')

	// Upload to the same path with replace strategy
	const response = await livinityd.api.post('files/upload?path=/Home/replace-test.txt&collision=replace', {
	...
	const content = await fse.readFile(`${livinityd.instance.dataDirectory}/home/replace-test.txt`, 'utf8')
