...
test('emptyTrash() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.files.emptyTrash.mutate()).rejects.toThrow('Invalid token')
})
...
test('emptyTrash() successfully empties an empty trash', async () => {
	...
	// Empty the trash
	const result = await livinityd.client.files.emptyTrash.mutate()
