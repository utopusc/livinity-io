...
export default class Search {
	...
	async search(query: string, maxResults = 250) {
		...
		for await (const systemPath of this.#livinityd.files.streamContents('/Home')) {
			// Grab the filename
			const filename = nodePath.basename(systemPath)

			// Skip hidden files
			if (this.#livinityd.files.isHidden(filename)) continue
		...
		// Get file objects
		let fileReads = await Promise.allSettled(results.map((result) => this.#livinityd.files.status(result.systemPath)))
