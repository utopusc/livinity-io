...
test('trash() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.files.trash.mutate({path: '/Home/Documents'})).rejects.toThrow(
		'Invalid token',
	)
})
...
test('trash() throws on directory traversal attempt', async () => {
	await expect(
		livinityd.client.files.trash.mutate({
			path: '/Home/../../../../etc',
		}),
...
test('trash() throws on relative paths', async () => {
	await Promise.all(
		['', ' ', '.', '..', 'Home', 'Home/..', 'Home/Documents'].map(async (path) =>
			expect(livinityd.client.files.trash.mutate({path})).rejects.toThrow('[path-not-absolute]'),
		),
	)
})
...
test('trash() throws on protected paths', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/Downloads`
...
test('trash() successfully moves a file to trash', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/trash-file-test`
	...
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}`)).resolves.toBe(true)
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/trash`)).resolves.toBe(true)
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/trash-meta`)).resolves.toBe(true)

	// Trash the file
	await expect(livinityd.client.files.trash.mutate({path: '/Home/trash-file-test/file.txt'})).resolves.toBe(
		'/Trash/file.txt',
	)
	...
	const trashSystemPath = `${livinityd.instance.dataDirectory}/trash/file.txt`
	...
	const metaPath = `${livinityd.instance.dataDirectory}/trash-meta/file.txt.json`
...
test('trash() successfully moves a directory with contents to trash', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/trash-directory-test`
	await fse.mkdir(testDirectory)
	...
	// Trash the directory
	await expect(livinityd.client.files.trash.mutate({path: '/Home/trash-directory-test/subdir'})).resolves.toBe(
		'/Trash/subdir',
	)
	...
	const trashSystemPath = `${livinityd.instance.dataDirectory}/trash/subdir`
	await expect(fse.pathExists(trashSystemPath)).resolves.toBe(true)
	...
	// Verify metadata file exists
	const metaPath = `${livinityd.instance.dataDirectory}/trash-meta/subdir.json`
	await expect(fse.pathExists(metaPath)).resolves.toBe(true)
...
test('trash() handles name conflicts by appending numbers', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/trash-conflict-test`
	await fse.mkdir(testDirectory)
	...
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/trash/file.txt`)).resolves.toBe(true)
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/trash/file (2).txt`)).resolves.toBe(true)

	// Verify metadata files exist with the correct name
	const metaPath = `${livinityd.instance.dataDirectory}/trash-meta/file.txt.json`
	await expect(fse.pathExists(metaPath)).resolves.toBe(true)
	const metaPath2 = `${livinityd.instance.dataDirectory}/trash-meta/file (2).txt.json`
	await expect(fse.pathExists(metaPath2)).resolves.toBe(true)

	// Clean up
	await fse.remove(testDirectory)
	await fse.remove(`${livinityd.instance.dataDirectory}/trash/file.txt`)
	await fse.remove(`${livinityd.instance.dataDirectory}/trash/file (2).txt`)
	await fse.remove(metaPath)
...
test('trash() handles trashing two files of the same name at the same time', async () => {
	// Create test directory and files
	const testDirectory = `${livinityd.instance.dataDirectory}/home/trash-conflict-async-test`
	await fse.mkdir(testDirectory)
	await fse.writeFile(`${testDirectory}/file.txt`, 'content1')
	await fse.mkdir(`${testDirectory}/subdir`)
...
test('trash() preserves symlinks when trashing', async () => {
	...
	await fse.mkdir(`${livinityd.instance.dataDirectory}/home/trash-symlink-test`, {recursive: true})
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/trash-symlink-test/target.txt`, 'target content')
	await fse.symlink(
		`${livinityd.instance.dataDirectory}/home/trash-symlink-test/target.txt`,
		`${livinityd.instance.dataDirectory}/home/trash-symlink-test/symlink`,
	)
	...
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/home/trash-symlink-test/symlink`)).resolves.toBe(false)

	// Verify it's still a symlink in trash
	const trashedSymlink = `${livinityd.instance.dataDirectory}/trash/symlink`
	...
	expect(linkTarget).toBe(`${livinityd.instance.dataDirectory}/home/trash-symlink-test/target.txt`)
	...
	// Clean up
	await fse.remove(`${livinityd.instance.dataDirectory}/home/trash-symlink-test`)
	await fse.remove(trashedSymlink)
	await fse.remove(`${livinityd.instance.dataDirectory}/trash-meta/symlink.json`)
})
