...
// Create a new livinityd instance for tests
...
beforeEach(async () => {
	...
	await fse.emptyDir(`${livinityd.instance.dataDirectory}/home`)
})
...
describe('archive()', () => {
	test('throws unauthorized error without auth token', async () => {
		await expect(livinityd.unauthenticatedClient.files.archive.mutate({paths: ['/Home/test.txt']})).rejects.toThrow(
			'Invalid token',
		)
	})
	...
	test('throws error on directory traversal attempt', async () => {
		await expect(
			livinityd.client.files.archive.mutate({
	...
	test('throws error when paths are in different directories', async () => {
		...
		await fse.ensureDir(`${livinityd.instance.dataDirectory}/home/dir1`)
		await fse.ensureDir(`${livinityd.instance.dataDirectory}/home/dir2`)
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/dir1/file1.txt`, 'content1')
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/dir2/file2.txt`, 'content2')
	...
	test('successfully creates a zip archive from a single file', async () => {
		// Create a test file
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/test-file.txt`, 'test content')

		// Archive the file
		const zipPath = await livinityd.client.files.archive.mutate({
			paths: ['/Home/test-file.txt'],
		})

		// Check archive file exists
		expect(zipPath).toBe('/Home/test-file.txt.zip')
		const zipFile = `${livinityd.instance.dataDirectory}/home/test-file.txt.zip`
	...
	test('successfully creates a zip archive from multiple files', async () => {
		// Create test files
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/file1.txt`, 'content1')
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/file2.txt`, 'content2')
		...
		const zipFile = `${livinityd.instance.dataDirectory}/home/Archive.zip`
		await expect(fse.pathExists(zipFile)).resolves.toBe(true)
	...
	test('successfully creates a zip archive from a directory', async () => {
		...
		await fse.ensureDir(`${livinityd.instance.dataDirectory}/home/test-dir`)
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/test-dir/file1.txt`, 'content1')
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/test-dir/file2.txt`, 'content2')
		await fse.ensureDir(`${livinityd.instance.dataDirectory}/home/test-dir/subdir`)
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/test-dir/subdir/file3.txt`, 'content3')
		...
		const zipFile = `${livinityd.instance.dataDirectory}/home/test-dir.zip`
		await expect(fse.pathExists(zipFile)).resolves.toBe(true)
	...
	test('creates a uniquely named zip archive when a file with the same name already exists', async () => {
		// Create a test file
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/unique-test.txt`, 'test content')

		// Create a zip file that would conflict with the generated name
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/unique-test.txt.zip`, 'existing zip')
		...
		const zipFile = `${livinityd.instance.dataDirectory}/home/unique-test.txt (2).zip`
		await expect(fse.pathExists(zipFile)).resolves.toBe(true)
		...
		const originalZipContent = await fse.readFile(`${livinityd.instance.dataDirectory}/home/unique-test.txt.zip`, 'utf8')
	...
	test('handles files with special characters in name', async () => {
		...
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/${fileName}`, 'special content')
		...
		const zipFile = `${livinityd.instance.dataDirectory}/home/${fileName}.zip`
		await expect(fse.pathExists(zipFile)).resolves.toBe(true)
	...
	test('handles empty directories correctly', async () => {
		...
		await fse.ensureDir(`${livinityd.instance.dataDirectory}/home/empty-dir`)
		...
		const zipFile = `${livinityd.instance.dataDirectory}/home/empty-dir.zip`
		await expect(fse.pathExists(zipFile)).resolves.toBe(true)
...
describe('unarchive()', () => {
	test('throws unauthorized error without auth token', async () => {
		await expect(livinityd.unauthenticatedClient.files.unarchive.mutate({path: '/Home/test.zip'})).rejects.toThrow(
			'Invalid token',
		)
	})
	...
	test('throws error on directory traversal attempt', async () => {
		await expect(
			livinityd.client.files.unarchive.mutate({
	...
	test('throws error on unsupported file format', async () => {
		...
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/test.txt`, 'This is not an archive')
	...
	test('extracts a zip archive correctly', async () => {
		...
		const zipFile = `${livinityd.instance.dataDirectory}/home/test-extract.zip`
		...
		// Extract the archive
		const extractPath = await livinityd.client.files.unarchive.mutate({
			path: '/Home/test-extract.zip',
		})

		// Verify extracted folder path
		expect(extractPath).toBe('/Home/test-extract')
		const extractDir = `${livinityd.instance.dataDirectory}/home/test-extract`
	...
	test('creates a unique directory name if target already exists', async () => {
		...
		await fse.ensureDir(`${livinityd.instance.dataDirectory}/home/conflict-test`)
		await fse.writeFile(`${livinityd.instance.dataDirectory}/home/conflict-test/existing-file.txt`, 'existing content')

		// Create a test zip file
		const zipFile = `${livinityd.instance.dataDirectory}/home/conflict-test.zip`
		const zip = new AdmZip()
		...
		await expect(
			fse.readFile(`${livinityd.instance.dataDirectory}/home/conflict-test/existing-file.txt`, 'utf8'),
		).resolves.toBe('existing content')
		await expect(
			fse.readFile(`${livinityd.instance.dataDirectory}/home/conflict-test (2)/new-file.txt`, 'utf8'),
	...
	test('handles files with special characters in name', async () => {
		...
		const zipFile = `${livinityd.instance.dataDirectory}/home/${fileName}`
		...
		const extractedFile = `${livinityd.instance.dataDirectory}/home/special & chars 漢字/test.txt`
	...
	for (const type of archiveTypes) {
		test(`extracts a ${type.extension} archive correctly`, async () => {
			const sourceDir = `${livinityd.instance.dataDirectory}/home/archive-test`
			const archiveFile = `${livinityd.instance.dataDirectory}/home/archive-test${type.extension}`
