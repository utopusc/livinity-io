...
// Phase 338 (RECYCLE-01, W7) — the macOS-compat VFS module chain, extracted to a
// SINGLE source of truth so the [global] section and the per-share recycle stanza
// can never drift (a per-share `vfs objects =` REPLACES the global chain rather
// than merging, so the recycle stanza must re-list every module here + `recycle`).
export const SMB_VFS_CHAIN = 'catia fruit streams_xattr'

// Phase 338 — default soft-delete policy (store-fallback, mirrors EMPTY_CONNECTIVITY_STATE).
export const DEFAULT_SMB_RECYCLE = {enabled: true, purgeDays: 30} as const
// Single-file cap: deletes larger than this bypass the bin (freed immediately) — the
// primary "silent disk-fill" guard (RESEARCH §1.2/§1.4). Constant in v1 (not a store
// field — smbRecycle schema is {enabled,purgeDays}); revisit if operators ask.
export const RECYCLE_MAXSIZE_BYTES = 10 * 1024 * 1024 * 1024 // 10 GiB
export const RECYCLE_BIN_DIRNAME = '.Recycle.Bin'
...
# Enable Time Machine backups.
fruit:time machine = yes
${renderRecycleStanza(recycleEnabled)}`

// Pure — the per-share vfs_recycle stanza. enabled=false → '' (byte-identical escape
// hatch: appending an empty string leaves perUserShareConfig's output unchanged).
// enabled=true → the FULL vfs chain (SMB_VFS_CHAIN + recycle — a per-share `vfs
// objects` REPLACES the global chain, so all modules must be listed to preserve
// macOS Fruit compat) + %U per-user repository + hide files.
export function renderRecycleStanza(enabled: boolean): string {
	if (!enabled) return ''
	return `
# SMB soft-delete (Phase 338 RECYCLE-01). Deletes land in ${RECYCLE_BIN_DIRNAME}/%U
# (the connecting user's own uid — no force user — so subdirs are 0700-isolated).
vfs objects = ${SMB_VFS_CHAIN} recycle
recycle:repository = ${RECYCLE_BIN_DIRNAME}/%U
...
recycle:minsize = 0
recycle:maxsize = ${RECYCLE_MAXSIZE_BYTES}
recycle:exclude = *.tmp/~$*/*.temp/*.log/*.part/.DS_Store/Thumbs.db/desktop.ini/*.crdownload/*.download
recycle:exclude_dir = /${RECYCLE_BIN_DIRNAME}//tmp//@eaDir//.streams/
hide files = /${RECYCLE_BIN_DIRNAME}/
`
}
...
export function renderShareBlock(
	...
	// Phase 338 — gate the vfs_recycle stanza (per-user blocks only). Default false so
	// the legacy path + existing pure unit-test call sites stay byte-identical.
	...
	return perUserShareConfig(name, path, perUser.validUsers, perUser.writeList, recycleEnabled)
}
...
export default class Samba {
	...
	async applyShares() {
		...
		// Phase 338 (RECYCLE-01, D-338-1) — global soft-delete policy read ONCE before
		// the loop; fail-soft to the default-ON constant on a missing/undecodable key.
		const recycle = (await this.#livinityd.store.get('smbRecycle').catch(() => undefined)) ?? DEFAULT_SMB_RECYCLE
		...
		for (const share of shares) {
			...
			// Recycle stanza is per-user-ONLY (D-338-1): legacy shared-account blocks
			// never carry it (would need a shared bin under force-user-root — out of
			// scope, and breaks the byte-identical-legacy guarantee).
			const recycleEnabled = recycle.enabled && perUser !== null

			// Append the share config (legacy shared-account block, or the per-user
			// block driven by the render-time-resolved grants).
			config += renderShareBlock(shareName, systemPath, perUser, recycleEnabled)

			// Best-effort top-level `.Recycle.Bin` provisioning (fail-soft — never
			// blocks config write / smbd start). Only for per-user shares with recycle on.
			if (recycleEnabled) await this.#ensureRecycleBin(systemPath)
		}
