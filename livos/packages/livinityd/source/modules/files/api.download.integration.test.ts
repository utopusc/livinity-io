...
test('GET /api/files/download throws unauthorized error whithout cookie', async () => {
	const error = await livinityd.unauthenticatedApi.get('files/download').catch((error) => error)
...
test('GET /api/files/download throws 400 error without path parameter', async () => {
	const error = await livinityd.api.get('files/download').catch((error) => error)
	expect(error).toBeInstanceOf(Error)
...
test('GET /api/files/download throws 404 error when file does not exist', async () => {
	const error = await livinityd.api.get('files/download?path=/Home/does-not-exist').catch((error) => error)
	expect(error).toBeInstanceOf(Error)
...
test('GET /api/files/download throws 404 error on directory traversal attempt', async () => {
	const error = await livinityd.api.get('files/download?path=/Home/../../../../etc/passwd').catch((error) => error)
	expect(error).toBeInstanceOf(Error)
	expect(error.response.statusCode).toBe(404)
	expect(error.response.body).toMatchObject({error: 'not found'})
})
...
test('GET /api/files/download throws 404 error on relative path', async () => {
	...
	for (const path of paths) {
		const error = await livinityd.api.get(`files/download?path=${path}`).catch((error) => error)
		expect(error).toBeInstanceOf(Error)
		expect(error.response.statusCode).toBe(404)
		expect(error.response.body).toMatchObject({error: 'not found'})
	}
...
test('GET /api/files/download throws 404 error on symlink traversal attempt', async () => {
	...
	const error = await livinityd.api.get('files/download?path=/Home/symlink-to-root/etc/passwd').catch((error) => error)
	expect(error).toBeInstanceOf(Error)
	expect(error.response.statusCode).toBe(404)
	expect(error.response.body).toMatchObject({error: 'not found'})
})
...
test('GET /api/files/download throws 404 error when one of multiple files does not exist', async () => {
	// Create one file
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/file.txt`, 'contents')
...
test('GET /api/files/download throws 400 error when paths are in different directories', async () => {
	...
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/dir1/file1.txt`, 'contents1')
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/dir2/file2.txt`, 'contents2')
...
test('GET /api/files/download downloads a file with a valid cookie', async () => {
	...
	// Download the file
	const response = await livinityd.api.get('files/download?path=/Home/file.txt', {responseType: 'text'})
...
test('GET /api/files/download downloads a file with special characters in name', async () => {
	...
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/${fileName}`, 'special contents')

	// Download the file
	const response = await livinityd.api.get(`files/download?path=/Home/${encodeURIComponent(fileName)}`, {
		responseType: 'text',
	})
...
test('GET /api/files/download creates a zip archive for multiple files', async () => {
	...
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/file1.txt`, 'contents1')
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/file2.txt`, 'contents2')

	// Download the files
	const response = await livinityd.api.get('files/download?path=/Home/file1.txt&path=/Home/file2.txt', {
		responseType: 'buffer',
	})
	...
	expect(response.headers['content-disposition']).toBe(`attachment; filename*=UTF-8''livinity-files.zip`)
...
test('GET /api/files/download creates a zip archive for a directory', async () => {
	...
	await fse.ensureDir(`${livinityd.instance.dataDirectory}/home/testdir`)
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/testdir/file1.txt`, 'contents1')
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/testdir/file2.txt`, 'contents2')
	await fse.ensureDir(`${livinityd.instance.dataDirectory}/home/testdir/subdir`)
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/testdir/subdir/file3.txt`, 'contents3')

	// Download the directory
	const response = await livinityd.api.get('files/download?path=/Home/testdir', {
		responseType: 'buffer',
	})
...
test('GET /api/files/download handles empty directories correctly', async () => {
	...
	// Download the directory
	const response = await livinityd.api.get('files/download?path=/Home/empty-dir', {
		responseType: 'buffer',
	})
...
test('GET /api/files/download handles files with zero content correctly', async () => {
	...
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/empty-file.txt`, '')

	// Download the file
	const response = await livinityd.api.get('files/download?path=/Home/empty-file.txt')
...
test('GET /api/files/download handles binary files correctly', async () => {
	...
	await fse.writeFile(`${livinityd.instance.dataDirectory}/home/binary-file.bin`, binaryData)

	// Download the file
	const response = await livinityd.api.get('files/download?path=/Home/binary-file.bin', {
		responseType: 'buffer',
	})
