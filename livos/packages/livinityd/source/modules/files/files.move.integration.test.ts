...
for (const forceSlowMoveWithProgress of forceSlowMoveWithProgressValues) {
	describe(`move() with LIVINITYD_FORCE_SLOW_MOVE_WITH_PROGRESS=${forceSlowMoveWithProgress}`, () => {
		beforeEach(() => {
			process.env.LIVINITYD_FORCE_SLOW_MOVE_WITH_PROGRESS = forceSlowMoveWithProgress
		})
		test('move() throws invalid error without auth token', async () => {
			await expect(
				livinityd.unauthenticatedClient.files.move.mutate({
					path: '/Home/Documents',
		...
		test('move() throws on directory traversal attempt in source path', async () => {
			await expect(
				livinityd.client.files.move.mutate({
					path: '/Home/../../../../etc',
					toDirectory: '/Home',
				}),
		...
		test('move() throws when moving a directory into a subdirectory of itself', async () => {
			testDirectory = `${livinityd.instance.dataDirectory}/home/inside-self-move-test`
		...
		test.each(['/Home', '/Apps', '/Home/Downloads'])(
			...
			async (path) => {
				testDirectory = `${livinityd.instance.dataDirectory}/home/protected-move-test`
				await fse.mkdir(testDirectory)
		...
		test('move() throws when trying to move a protected path out of /Apps/', async () => {
			...
			testDirectory = `${livinityd.instance.dataDirectory}/home/protected-app-move-test`
			await fse.mkdir(testDirectory)
			...
			// Clean up
			await fse.remove(testDirectory)
			await livinityd.client.apps.uninstall.mutate({appId: 'sparkles-hello-world'})
		})

		test('move() does not throw when moving an unprotected path out of /Apps/', async () => {
			testDirectory = `${livinityd.instance.dataDirectory}/home/unprotected-apps-move-test`
			await fse.mkdir(testDirectory)

			// create a directory in /Apps/ that is not an installed app id
			await fse.mkdir(`${livinityd.instance.dataDirectory}/app-data/not-an-app-id`)
		...
		test('move() throws on too many duplicate names from existing paths when using the "keep-both" collision strategy', async () => {
			...
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-existing-test`
			await fse.mkdir(testDirectory)
		...
		test('move() moves a single file to a directory', async () => {
			// Create test directory and file
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-file-test`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
			...
			// Move the file
			const result = await livinityd.client.files.move.mutate({
		...
		test('move() uses correct move operation', async () => {
			// Create test directory and file
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-test-impl`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
			await fse.writeFile(`${testDirectory}/source/source.txt`, 'test content')
			await fse.mkdir(`${testDirectory}/destination`)
			...
			if (forceSlowMoveWithProgress === 'true') {
				// When LIVINITYD_FORCE_SLOW_MOVE_WITH_PROGRESS is true, rsync should be used
				expect(execa).toHaveBeenCalledWith('rsync', expect.anything())
			} else {
				// When LIVINITYD_FORCE_SLOW_MOVE_WITH_PROGRESS is false, normal move should be used
		...
		test('move() handles moving a file to the current containing directory by doing nothing', async () => {
			// Create test directory and file
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-same-dir-file-test`
			await fse.mkdir(testDirectory)
		...
		test('move() handles moving a file to a different directory by throwing on name conflict by default', async () => {
			...
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-file-conflict-test`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
		...
		test('move(path, {collision: "keep-both"}) keeps both files by appending a number to the moved file', async () => {
			// Create test directory structure
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-keep-both-test`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
			await fse.writeFile(`${testDirectory}/source/file.txt`, 'source content')
			await fse.mkdir(`${testDirectory}/destination`)
		...
		test('move(path, {collision: "replace"}) replaces the existing file with the moved file', async () => {
			// Create test directory structure
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-replace-test`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
			await fse.writeFile(`${testDirectory}/source/file.txt`, 'source content')
			await fse.mkdir(`${testDirectory}/destination`)
		...
		test('move() moves a directory with contents', async () => {
			// Create test directory structure
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-directory-test`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
		...
		test('move() handles moving a directory to the current containing directory by doing nothing', async () => {
			// Create test directory structure
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-same-dir-test`
			await fse.mkdir(testDirectory)
		...
		test('move() handles moving a directory to a different directory by throwing on name conflict by default', async () => {
			// Create test directory structure
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-dir-conflict-test`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
			await fse.writeFile(`${testDirectory}/source/file.txt`, 'source content')
		...
		test('move(path, {collision: "keep-both"}) keeps both directories by appending a number to the moved directory', async () => {
			// Create test directory structure
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-dir-keep-both-test`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
			await fse.writeFile(`${testDirectory}/source/file.txt`, 'source content')
			await fse.mkdir(`${testDirectory}/destination`)
		...
		test('move(path, {collision: "replace"}) replaces the existing directory with the moved directory', async () => {
			// Create test directory structure
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-dir-replace-test`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
		...
		test('move() moves symlinks as symlinks', async () => {
			// Create test directory and file
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-symlink-test`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
		...
		test('move() preserves file permissions, ownership and timestamps', async () => {
			// Create test directory and file
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-permissions-test`
			await fse.mkdir(testDirectory)
			await fse.mkdir(`${testDirectory}/source`)
			...
			// Set specific ownership (use livinity user ID from files class)
			const uid = 1234
			const gid = 1234
		...
		test('move() to same directory is a no-op', async () => {
			// Create test directory and file
			testDirectory = `${livinityd.instance.dataDirectory}/home/move-same-directory-test`
			await fse.mkdir(testDirectory)
