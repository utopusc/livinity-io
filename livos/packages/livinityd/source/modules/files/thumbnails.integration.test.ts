...
// Create new livinityd instance for each test
beforeEach(async () => {
...
describe('getThumbnail', () => {
	test('throws invalid error without auth token', async () => {
		await expect(livinityd.unauthenticatedClient.files.getThumbnail.mutate({path: '/Home/test.jpg'})).rejects.toThrow(
			'Invalid token',
		)
	})
	...
	test('returns a correctly formatted api endpoint URL for a thumbnail', async () => {
		// Create test image
		const testDir = `${livinityd.instance.dataDirectory}/home/thumbnail-test`
		...
		const thumbnailUrl = await livinityd.client.files.getThumbnail.mutate({path: virtualPath})
	...
	for (const imageType of imageTypes) {
		test(`returns a thumbnail for a ${imageType} file`, async () => {
			...
			const thumbnailFilename = thumbnailUrl.split('/').pop()
			const thumbnailPath = `${livinityd.instance.dataDirectory}/thumbnails/${thumbnailFilename}`
			const thumbnailExists = await fse.pathExists(thumbnailPath)
			expect(thumbnailExists).toBe(true)
		})
	}
	...
	test('returns same thumbnail for renamed files', async () => {
		...
		const virtualPath = '/Home/thumbnail-test/master-lossless-image.png'
		const originalThumbnailUrl = await livinityd.client.files.getThumbnail.mutate({path: virtualPath})
		...
		const renamedThumbnailUrl = await livinityd.client.files.getThumbnail.mutate({path: renamedVirtualPath})
	...
	test('returns same thumbnail for moved files', async () => {
		...
		const sourceDir = `${livinityd.instance.dataDirectory}/home/thumbnail-test/source`
		const destDir = `${livinityd.instance.dataDirectory}/home/thumbnail-test/destination`
		...
		const movedThumbnailUrl = await livinityd.client.files.getThumbnail.mutate({path: movedVirtualPath})
	...
	test('generates a new thumbnail when source file is modified', async () => {
		...
		const originalThumbnailPath = `${livinityd.instance.dataDirectory}/thumbnails/${originalThumbnailFilename}`
		...
		const modifiedThumbnailUrl = await livinityd.client.files.getThumbnail.mutate({path: modifiedVirtualPath})
		...
		const newThumbnailPath = `${livinityd.instance.dataDirectory}/thumbnails/${newThumbnailFilename}`
	...
	test('returns existing thumbnails when they exist without generating a new one', async () => {
		...
		const virtualPath = '/Home/thumbnail-test/master-lossless-image.png'
		const thumbnailUrl1 = await livinityd.client.files.getThumbnail.mutate({path: virtualPath})
		...
		const thumbnailUrl2 = await livinityd.client.files.getThumbnail.mutate({path: virtualPath})
...
describe('Background file watcher', () => {
	test('generates thumbnails for new image files', {retry: 5}, async () => {
		const testDir = `${livinityd.instance.dataDirectory}/home/thumbnail-watcher-test`
		await fse.ensureDir(testDir)
	...
	test('does not update thumbnails when files are moved', {retry: 5}, async () => {
		// Create test directories
		const sourceDir = `${livinityd.instance.dataDirectory}/home/thumbnail-watcher-move-test/source`
		const destDir = `${livinityd.instance.dataDirectory}/home/thumbnail-watcher-move-test/destination`
		await fse.ensureDir(destDir)
	...
	test('does not update thumbnails when files are renamed', {retry: 5}, async () => {
		// Create test directory
		const testDir = `${livinityd.instance.dataDirectory}/home/thumbnail-watcher-rename-test`
		await fse.ensureDir(testDir)
	...
	test('updates thumbnails when files are modified', {retry: 5}, async () => {
		// Create test directory and image
		const testDir = `${livinityd.instance.dataDirectory}/home/thumbnail-watcher-modify-test`
		const imagePath = await copyFixtureFile(testDir)
...
describe('files.list() [thumbnail specific]', () => {
	test('includes thumbnail for a file with an existing thumbnail', async () => {
		...
		const files = await livinityd.client.files.list.query({path: '/Home/thumbnail-test'})
...
describe('recents() [thumbnail specific]', () => {
	test('includes thumbnail for a recent file with an existing thumbnail', {retry: 5}, async () => {
		// Create test directory
		const testDir = `${livinityd.instance.dataDirectory}/home/recents-thumbnail-test`
		await fse.ensureDir(testDir)
		...
		// Get recent files
		const recentFiles = await livinityd.client.files.recents.query()
...
describe('Thumbnail housekeeping', () => {
	test('removes oldest thumbnails when exceeding cleanup threshold', {retry: 5}, async () => {
		...
		const thumbnailsInstance = livinityd.instance.files.thumbnails
		thumbnailsInstance.maxThumbnailCount = 20
		thumbnailsInstance.pruningThreshold = 10

		// Create test images directory
		const testDir = `${livinityd.instance.dataDirectory}/home/thumbnail-cleanup-test`
	...
	test('removes excess thumbnails on startup', {retry: 5}, async () => {
		...
		livinityd.instance.files.thumbnails.maxThumbnailCount = maxThumbnailCount
		...
		// Now start the livinity instance - this should trigger the cleanup on startup
...
describe('Queue selection', () => {
	test('uses background queue for watcher events and on-demand queue for explicit requests', {retry: 5}, async () => {
		// Create test directory
		const testDir = `${livinityd.instance.dataDirectory}/home/thumbnail-queue-test`
		await fse.ensureDir(testDir)
		...
		await livinityd.client.files.getThumbnail.mutate({path: virtualPath})
