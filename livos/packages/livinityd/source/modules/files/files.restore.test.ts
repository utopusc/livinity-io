...
afterEach(async () => {
	...
	const trashDir = `${livinityd.instance.dataDirectory}/trash`
	const trashMetaDir = `${livinityd.instance.dataDirectory}/trash-meta`
...
test('restore() throws invalid error without auth token', async () => {
	...
	await expect(livinityd.unauthenticatedClient.files.restore.mutate({path: '/Trash/foo'})).rejects.toThrow(
		'Invalid token',
	)
})
...
test('restore() throws on directory traversal attempt', async () => {
	await expect(
		livinityd.client.files.restore.mutate({
			path: '/Trash/../../../../etc',
		}),
...
test('restore() throws on symlink traversal attempt', async () => {
	...
	await $`ln -s / ${livinityd.instance.dataDirectory}/trash/symlink-to-root`
	...
	// Clean up
	await fse.remove(`${livinityd.instance.dataDirectory}/trash/symlink-to-root`)
})
...
test('restore() throws when metadata file is missing', async () => {
	...
	await fse.writeFile(`${livinityd.instance.dataDirectory}/trash/no-meta.txt`, 'content')
	...
	// Clean up
	await fse.remove(`${livinityd.instance.dataDirectory}/trash/no-meta.txt`)
})

test('restore() successfully restores a file from trash', async () => {
	// Create test directory and file
	const testDirectory = `${livinityd.instance.dataDirectory}/home/restore-file-test`
	...
	const trashPath = await livinityd.client.files.trash.mutate({
		path: virtualPath,
	})
	...
	await expect(livinityd.client.files.restore.mutate({path: trashPath})).resolves.toBe(virtualPath)

	// Verify the file no longer exists in trash
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/trash/restore-file.txt`)).resolves.toBe(false)
	...
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/trash-meta/restore-file.txt.json`)).resolves.toBe(
		false,
	)
...
test('restore() successfully restores a directory with contents from trash', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/restore-dir-test`
	await fse.mkdir(testDirectory, {recursive: true})
	...
	const restoredPath = await livinityd.client.files.restore.mutate({
		path: trashPath,
	})

	// Verify the directory is moved from trash
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/trash/restore-dir`)).resolves.toBe(false)
	...
	// Verify metadata file is deleted
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/trash-meta/restore-dir.json`)).resolves.toBe(false)
...
test('restore() successfully restores a child directory from trash', async () => {
	// Create test directory structure
	const testDirectory = `${livinityd.instance.dataDirectory}/home/restore-child-dir-test`
	await fse.mkdir(testDirectory, {recursive: true})
	...
	// Verify the directory is moved from trash
	await expect(
		fse.pathExists(`${livinityd.instance.dataDirectory}/trash/restore-child-dir-test/child-dir`),
	).resolves.toBe(false)
...
test('restore() successfully restores multiple subdirectories from trash one after another', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/restore-multiple-subdirs-test`
	await fse.mkdir(testDirectory, {recursive: true})
	...
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/trash/restore-multiple-subdirs-test`)).resolves.toBe(
		true,
	)

	// Now restore the first subdirectory
	const restoredPath1 = await livinityd.client.files.restore.mutate({
	...
	await expect(
		fse.pathExists(`${livinityd.instance.dataDirectory}/trash/restore-multiple-subdirs-test/subdir2`),
	).resolves.toBe(true)

	// Now restore the second subdirectory
	const restoredPath2 = await livinityd.client.files.restore.mutate({
...
test('restore() throws on name conflict', async () => {
	// Create test directory and file
	const testDirectory = `${livinityd.instance.dataDirectory}/home/conflict-test`
	await fse.mkdir(testDirectory, {recursive: true})
...
test('restore(path, {collision: "keep-both"}) handles name conflict', async () => {
	// Create test directory and file
	const testDirectory = `${livinityd.instance.dataDirectory}/home/conflict-keep-both-test`
	await fse.mkdir(testDirectory, {recursive: true})
	await fse.writeFile(`${testDirectory}/conflict-file.txt`, 'original content')
...
test('restore(path, {collision: "replace"}) handles name conflict', async () => {
	// Create test directory and file
	const testDirectory = `${livinityd.instance.dataDirectory}/home/conflict-replace-test`
	await fse.mkdir(testDirectory, {recursive: true})
	await fse.writeFile(`${testDirectory}/conflict-file.txt`, 'original content')
	...
	// Verify the file no longer exists in trash
	await expect(fse.pathExists(`${livinityd.instance.dataDirectory}/trash/conflict-file.txt`)).resolves.toBe(false)
...
test('restore(path, {collision: "replace"}) with directory replaces entire target directory instead of merging', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/dir-replace-test`
...
test('restore() creates parent directories if they do not exist', async () => {
	// Create test directory and file
	const testDirectory = `${livinityd.instance.dataDirectory}/home/parent-test`
	await fse.mkdir(testDirectory, {recursive: true})
...
test('restore() preserves symlinks when restoring directories', async () => {
	...
	const testDirectory = `${livinityd.instance.dataDirectory}/home/symlink-test`
	await fse.mkdir(testDirectory, {recursive: true})
