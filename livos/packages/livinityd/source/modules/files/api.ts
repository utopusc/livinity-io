...
export default function api({publicApi, privateApi, livinityd}: ApiOptions) {
	...
	privateApi.post('/upload', async (request, response) => {
		...
		// It's clear that a partially uploaded file with the .livinity-upload suffix is not a
		// completed upload.
		...
		// .somefile.jpg.livinity-upload not somefile.jpg so we don't trigger loads of
