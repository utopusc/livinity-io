...
test('rename() throws invalid error without auth token', async () => {
	await expect(
		livinityd.unauthenticatedClient.files.rename.mutate({path: '/Home/Documents', newName: 'Documents-copy'}),
	).rejects.toThrow('Invalid token')
})

test('rename() throws when newName is empty', async () => {
	const testDir = `${livinityd.instance.dataDirectory}/home/rename-test-empty`
	...
	await expect(
		livinityd.client.files.rename.mutate({
...
test('rename() throws on protected paths', async () => {
	...
	const testDir = `${livinityd.instance.dataDirectory}/home/Downloads`
	await fse.mkdir(testDir, {recursive: true})
...
test('rename() throws when source file/directory does not exist', async () => {
	...
	const testDir = `${livinityd.instance.dataDirectory}/home/rename-nonexistent`
	await fse.mkdir(testDir, {recursive: true})
...
test('rename() throws when destination already exists', async () => {
	const testDir = `${livinityd.instance.dataDirectory}/home/rename-test-unique`
	await fse.mkdir(testDir, {recursive: true})
...
test('rename() throws on filename traversal attack', async () => {
	const testDir = `${livinityd.instance.dataDirectory}/home/rename-test-invalid`
	await fse.mkdir(testDir, {recursive: true})
...
test('rename() renames a file successfully', async () => {
	const testDir = `${livinityd.instance.dataDirectory}/home/rename-test-file`
	await fse.mkdir(testDir, {recursive: true})
	...
	const result = await livinityd.client.files.rename.mutate({
		path: virtualOriginalPath,
		newName: 'renamed.txt',
	})
...
test('rename() renames a directory successfully', async () => {
	const testDir = `${livinityd.instance.dataDirectory}/home/rename-test-dir`
	await fse.mkdir(testDir, {recursive: true})
...
test('rename() returns the same path when newName is identical to current name', async () => {
	const testDir = `${livinityd.instance.dataDirectory}/home/rename-test-same`
	await fse.mkdir(testDir, {recursive: true})
...
test('rename() renames a symlink without altering its target', async () => {
	const testDir = `${livinityd.instance.dataDirectory}/home/rename-test-symlink`
	await fse.mkdir(testDir, {recursive: true})
