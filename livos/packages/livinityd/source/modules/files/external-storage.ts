...
export default class ExternalStorage {
	...
	async start() {
		...
		this.#removeDeviceChangeListener = this.#livinityd.eventBus.on('system:disk:change', async () => {
		...
		return this.#mountQueue.add(async () => {
			...
			for (const device of externalStorageDevices) {
				...
				for (const partition of device.partitions) {
					...
					try {
						// Derive mountpoint
						const externalBaseSystemPath = this.#livinityd.files.getBaseDirectory('/External')
						...
						systemMountpoint = await this.#livinityd.files.getUniqueName(systemMountpoint)

						// Mount partition
						await fse.ensureDir(systemMountpoint)
						await this.#livinityd.files.chownSystemPath(systemMountpoint)
						...
						this.#livinityd.eventBus.emit('files:external-storage:change')
					}
				}
			}
		})
		...
		for (const device of externalBlockDevices) {
			...
			for (const partition of device.partitions) {
				...
				partition.mountpoints = partition.mountpoints
					...
					.map((mountpoint) => this.#livinityd.files.systemToVirtualPath(mountpoint))
			}
	...
	// Get all livinityd mounted external devices
	...
	async isExternalDeviceConnectedOnUnsupportedDevice() {
		...
		const df = await $`df ${this.#livinityd.dataDirectory} --output=source`
