...
process.env.LIVINITYD_FORCE_SLOW_MOVE_WITH_PROGRESS = 'true'
// This forces 100 KB/s copies so we can test progress
process.env.LIVINITYD_FORCE_100KBS_COPY = 'true'

describe(`operationProgress()`, () => {
	test('throws invalid error without auth token', async () => {
		await expect(livinityd.unauthenticatedClient.files.operationProgress.query()).rejects.toThrow('Invalid token')
	})

	test('returns an empty array if no operations are in progress', async () => {
		await expect(livinityd.client.files.operationProgress.query()).resolves.toMatchObject([])
	})

	test('returns copy progress', async () => {
		// Create test directory and file
		const testDirectory = `${livinityd.instance.dataDirectory}/home/test-copy-progress`
		...
		// Copy the file
		const copyPromise = livinityd.client.files.copy.mutate({
	...
	test('returns move progress during inter filesystem move', async () => {
		// Create test directory and file
		const testDirectory = `${livinityd.instance.dataDirectory}/home/test-move-progress`
		await fse.mkdir(testDirectory)
		await fse.mkdir(`${testDirectory}/source`)
		const KB = 1024
		await fse.writeFile(`${testDirectory}/source/source.bin`, Buffer.alloc(100 * KB))
		...
		// Move the file
		const movePromise = livinityd.client.files.move.mutate({
