// Phase 102-06 — multi-instance Luse MCP config tests (renamed P100-10-02
// from bytebot per D-100-10-B; per-WebApp scoping now by X11 display, not wid).
//
// Coverage:
//   T1 — buildLuseConfig({}, path) returns the host-display shape (name=
//        'luse', no LUSE_TARGET_DISPLAY).
//   T2 — buildLuseConfig({}, path, descriptor) returns a per-WebApp
//        variant: name=`luse:webapp:<instanceKey>`, env carries the
//        descriptor.display under LUSE_TARGET_DISPLAY_ENV, instanceKey
//        separates two simultaneous instances.
//   T3 — Two descriptors with different displays produce distinct configs.
//   T4 — PER_WEBAPP_LUSE_INSTANCE_CAP is exported and equals 3.
//   T5 — LUSE_TARGET_DISPLAY_ENV constant is exported.
...
describe('buildLuseConfig multi-instance support (P102-06)', () => {
	it('T1: no descriptor → host-display name "luse", no LUSE_TARGET_DISPLAY env', () => {
	...
	it('T2: with descriptor → namespaced name + LUSE_TARGET_DISPLAY env', () => {
		const cfg = buildLuseConfig({DISPLAY: ':0'} as NodeJS.ProcessEnv, PATH, {
	...
	it('T3: two distinct descriptors produce different configs', () => {
		...
		expect(a.env?.[LUSE_TARGET_DISPLAY_ENV]).toBe(':10')
		expect(b.env?.[LUSE_TARGET_DISPLAY_ENV]).toBe(':11')
	})
	...
	it('T5: LUSE_TARGET_DISPLAY_ENV constant exported', () => {
