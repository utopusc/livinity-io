/**
 * Tests for luse-system-prompt.ts (renamed P100-10-02 from bytebot-system-prompt
 * per D-100-10-B; Plan 72-02 original)
 *
 ...
 *   2. Coordinate space anchor at 1280x960 (matches P71 RESOLUTION env)
 ...
 *   and `.toContain('COMPLETED')` (uppercase). Upstream prompt actually uses
...
describe('LUSE_SYSTEM_PROMPT', () => {
	it('is a non-empty string > 500 chars (Luse prompt is multi-paragraph)', () => {
		expect(typeof LUSE_SYSTEM_PROMPT).toBe('string');
		expect(LUSE_SYSTEM_PROMPT.length).toBeGreaterThan(500);
	});

	it('contains "You are Liv" (D-12 edit 1 applied)', () => {
		expect(LUSE_SYSTEM_PROMPT).toContain('You are Liv');
	});

	it('does NOT contain "You are Bytebot" (D-12 edit 1 reverse — old self-reference removed)', () => {
		expect(LUSE_SYSTEM_PROMPT).not.toContain('You are Bytebot');
	});
	...
	it('contains coordinate space anchor 1280x960 (D-12 edit 2)', () => {
		...
		expect(LUSE_SYSTEM_PROMPT).toMatch(/1280\s*x\s*960/);
	});
	...
	it('contains NEEDS_HELP and COMPLETED state sentinels (D-12 edit 3 — upstream lowercase verbatim)', () => {
		...
		expect(LUSE_SYSTEM_PROMPT).toContain('needs_help');
		expect(LUSE_SYSTEM_PROMPT).toContain('completed');
	});
	...
	it('contains no exotic Unicode that would break Kimi tokenizer (no emoji / RTL / private-use)', () => {
		...
		expect(LUSE_SYSTEM_PROMPT).not.toMatch(exotic);
	});

	it('preserves upstream tooling-name references (D-12 guard — only agent self-ref renamed)', () => {
		// Tool/UI names like "computer_screenshot" must remain — these refer to the
		// underlying tooling, NOT the agent identity (D-12 explicit guard).
		...
		expect(LUSE_SYSTEM_PROMPT).toContain('computer_screenshot');
		expect(LUSE_SYSTEM_PROMPT).toContain('set_task_status');
	});
});
