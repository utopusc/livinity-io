...
// Also carries mcp.config.installLivTools — the "One-Click: Install Liv MCPs"
// button (scripts/aionui-patches/install-liv-mcps-section.js) — and its CLI-picker
// sibling mcp.config.installLivMcpsToCli (writes the Liv MCPs into ONE chosen CLI's
// own config). Same rationale: both are adminProcedures with a bounded input (zero
// args / a single whitelisted CLI name), and they MUST reach livinityd :8080;
// without this exact-path carve-out they fall through to @liv (:3020 AionUi) and
// return the SPA index.html → `Unexpected token '<'` in the panel. EXACT paths (no
// wildcard) keep the LIVOS-054 batch-path boundary intact.
const LIV_CLI_INSTALLER_HANDLE = `\t@liv_cli_installer path /liv/trpc/cliInstaller.detect /liv/trpc/cliInstaller.install /liv/trpc/cliInstaller.auth /liv/trpc/cliInstaller.applyAgentChanges /liv/trpc/cliInstaller.hasPendingAgentChanges /liv/trpc/mcp.config.installLivTools /liv/trpc/mcp.config.installLivMcpsToCli
