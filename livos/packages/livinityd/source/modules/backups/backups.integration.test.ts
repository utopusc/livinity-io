...
beforeEach(async () => {
	process.env.LIVINITYD_RESTORE_SKIP_REBOOT = 'true'
	livinityd = await createTestLivinityd()
	await livinityd.registerAndLogin()
})
afterEach(async () => livinityd.cleanup())

// Create a `/Home/Backups`, directory, share it via Samba, and then mount it back
// to ourself as a network share at `/Network/localhost/Backups (Livinity)`
async function createBackupShare(livinityd: Awaited<ReturnType<typeof createTestLivinityd>>, directoryName = 'Backups') {
	await livinityd.client.files.createDirectory.mutate({path: `/Home/${directoryName}`})
	await livinityd.client.files.addShare.mutate({path: `/Home/${directoryName}`})
...
test('createRepository() throws on system path', async () => {
	...
	await expect(
		livinityd.client.backups.createRepository.mutate({
			path: `${livinityd.instance.dataDirectory}`,
...
test('createRepository() creates and stores a repository on external storage', async () => {
	...
	await expect(livinityd.client.backups.getRepositories.query()).resolves.toEqual([])

	// Create fake usb drive
	await fse.mkdir(`${livinityd.instance.dataDirectory}/external/SanDisk`, {recursive: false})

	// Create a new backup repository
	const repositoryId = await livinityd.client.backups.createRepository.mutate({
		path: `/External/SanDisk`,
		password: 'test-password',
	})

	// Verify repository was stored
	const repositories = await livinityd.client.backups.getRepositories.query()
...
test('createRepository() creates and stores a repository on network storage', async () => {
	...
	const backupNetworkSharePath = await createBackupShare(livinityd)
...
test('getRepositories() returns repositories with lastBackup date', async () => {
	...
	expect(repositories).toMatchObject([
		expect.objectContaining({
			...
			path: `${backupNetworkSharePath}/Livinity Backup.backup`,
		}),
	])
	expect(repositories[0].lastBackup).toBeUndefined()

	// Do a backup
	await livinityd.client.backups.backup.mutate({repositoryId})

	// Check we have a repository with a lastBackup date
	await expect(livinityd.client.backups.getRepositories.query()).resolves.toMatchObject([
		expect.objectContaining({
			id: expect.stringMatching(/[a-f0-9]{8}$/),
			path: `${backupNetworkSharePath}/Livinity Backup.backup`,
...
test('forgetRepository() forgets a repository', async () => {
	...
	// Verify repository was stored
	await expect(livinityd.client.backups.getRepositories.query()).resolves.toHaveLength(1)

	// Forget the repository
	await livinityd.client.backups.forgetRepository.mutate({repositoryId})

	// Verify repository was forgotten
	await expect(livinityd.client.backups.getRepositories.query()).resolves.toHaveLength(0)
})
...
test('backup() creates a backup successfully', async () => {
	...
	await expect(livinityd.client.backups.listBackups.query({repositoryId})).resolves.toHaveLength(0)

	// Do the backup
	await expect(livinityd.client.backups.backup.mutate({repositoryId})).resolves.toBe(true)

	// Verify backup was created by listing backups
	const backups = await livinityd.client.backups.listBackups.query({repositoryId})
	...
	const files = await livinityd.client.backups.listBackupFiles.query({backupId: backups[0].id})
	expect(files).toContain('livinity.yaml')
...
test('backup() throws error for non-existent repository', async () => {
	await expect(livinityd.client.backups.backup.mutate({repositoryId: 'non-existent-repo'})).rejects.toThrow(
		'Repository non-existent-repo not found',
	)
})

test('listBackups() throws error for non-existent repository', async () => {
	await expect(livinityd.client.backups.listBackups.query({repositoryId: 'non-existent-repo'})).rejects.toThrow(
		'Repository non-existent-repo not found',
	)
})
...
test('createRepository() prevents duplicate repositories in store', async () => {
	...
	// Create a new backup repository
	await livinityd.client.backups.createRepository.mutate({
		path: backupNetworkSharePath,
		password: 'test-password',
	})
...
test('multiple backups create separate snapshots', async () => {
	// Create test data
	const testDataPath = `${livinityd.instance.dataDirectory}/home/multi-backup-test.txt`
	await fse.mkdir(`${livinityd.instance.dataDirectory}/home`, {recursive: true})
	...
	// Create first backup
	await livinityd.client.backups.backup.mutate({repositoryId})
	const firstBackups = await livinityd.client.backups.listBackups.query({repositoryId})
	...
	await livinityd.client.backups.backup.mutate({repositoryId})
	const secondBackups = await livinityd.client.backups.listBackups.query({repositoryId})
...
test('mountBackup() mounts and unmountBackup() unmounts a backup', async () => {
	...
	const homeTestPath = `${livinityd.instance.dataDirectory}/home/test-home-file.txt`
	const appDataTestPath = `${livinityd.instance.dataDirectory}/app-data/test-app-file.txt`
	...
	await livinityd.client.backups.mountBackup.mutate({backupId: backup.id})

	await setTimeout(1000)

	// Verify that the backup root directory structure was created
	const {backupRoot} = livinityd.instance.backups
	...
	await livinityd.client.backups.unmountBackup.mutate({directoryName: dateDirs[0]})
...
test('listAllBackups() returns backups from all repositories', async () => {
	...
	const repositoryId1 = await livinityd.client.backups.createRepository.mutate({
		path: await createBackupShare(livinityd),
		password: 'test-password',
	})
	const repositoryId2 = await livinityd.client.backups.createRepository.mutate({
		path: await createBackupShare(livinityd, 'Backups 2'),
		password: 'test-password',
	})

	// Create a backup in the first repository
	await livinityd.client.backups.backup.mutate({repositoryId: repositoryId1})

	// Create a backup in the second repository
	await livinityd.client.backups.backup.mutate({repositoryId: repositoryId2})

	// Create another backup in the first repository
	await livinityd.client.backups.backup.mutate({repositoryId: repositoryId1})

	// Test listAllBackups
	const allBackups = await livinityd.client.backups.listAllBackups.query()
...
test('backup() reports progress during backup operation', async () => {
	...
	// Create 5MB file
	const sourceDirectory = `${livinityd.instance.dataDirectory}/home/orig`
	...
	const destinationDirectory = `${livinityd.instance.dataDirectory}/home/slow`
	...
	const removeListener = livinityd.instance.eventBus.on(
	...
	await expect(livinityd.client.backups.backupProgress.query()).resolves.toMatchObject([])
...
test('backupOnInterval() backs up in the background on an interval', async () => {
	// Set frequent backup interval and restart livinityd
	livinityd.instance.backups.backupInterval = 100 // 100ms
...
test('backups respect user ignored paths', async () => {
	...
	let backups = await livinityd.client.backups.listBackups.query({repositoryId})
	expect(backups).toHaveLength(1)
	let files = await livinityd.client.backups.listBackupFiles.query({backupId: backups[0].id})
	expect(files).toContain('home')

	// Check /Home is not listed as ignored
	await expect(livinityd.client.backups.getIgnoredPaths.query()).resolves.not.toContain('/Home')

	// Ignore /Home
	await expect(livinityd.client.backups.addIgnoredPath.mutate({path: '/Home'})).resolves.toBe(true)

	// Check /Home is listed as ignored
	await expect(livinityd.client.backups.getIgnoredPaths.query()).resolves.toContain('/Home')

	// Check ignoring non home path throws
	await expect(livinityd.client.backups.addIgnoredPath.mutate({path: '/App/foo'})).rejects.toThrow(
	...
	backups = await livinityd.client.backups.listBackups.query({repositoryId})
	expect(backups).toHaveLength(2)
	files = await livinityd.client.backups.listBackupFiles.query({backupId: backups[1].id})
	expect(files).not.toContain('home')

	// Remove ignored path
	await expect(livinityd.client.backups.removeIgnoredPath.mutate({path: '/Home'})).resolves.toBe(true)

	// Check /Home is no longer listed as ignored
	await expect(livinityd.client.backups.getIgnoredPaths.query()).resolves.not.toContain('/Home')

	// Check removing non home path throws
	await expect(livinityd.client.backups.removeIgnoredPath.mutate({path: '/External'})).rejects.toThrow(
		'Path to exclude must be in /Home',
	)
	...
	files = await livinityd.client.backups.listBackupFiles.query({backupId: backups[2].id})
	expect(files).toContain('home')
})

test('backups respect app backupIgnore glob patterns', async () => {
	// Install app
	await expect(livinityd.client.apps.install.mutate({appId: 'sparkles-hello-world'})).resolves.toStrictEqual(true)

	// Create files in the app's data directory
	const appDataDir = `${livinityd.instance.dataDirectory}/app-data/sparkles-hello-world`
	...
	const manifestPath = `${appDataDir}/livinity-app.yml`
	...
	const appDirFiles = await livinityd.client.backups.listBackupFiles.query({
	...
	const logsDirFiles = await livinityd.client.backups.listBackupFiles.query({
		backupId: backups[0].id,
	...
	const importantDirFiles = await livinityd.client.backups.listBackupFiles.query({
		backupId: backups[0].id,
...
test('backups handle disconnected network shares gracefully', async () => {
	...
	// Create a network share and mount it
	const mountPath = await createBackupShare(livinityd)

	// Verify the share is mounted and accessible
	await livinityd.client.files.list.query({path: mountPath})
	...
	await expect(livinityd.client.backups.listBackups.query({repositoryId})).resolves.toHaveLength(1)

	// Remove the share
	await livinityd.client.files.removeShare.mutate({path: '/Home/Backups'})
	...
	await expect(livinityd.client.backups.backup.mutate({repositoryId})).rejects.toThrow('host is down')

	// Add the share again
	await livinityd.client.files.addShare.mutate({path: '/Home/Backups'})

	// Attempt another backup
	await pRetry(() => expect(livinityd.client.backups.backup.mutate({repositoryId})).resolves.toBe(true), {
		retries: 5,
		factor: 1,
	})

	// Verify a second backup was created
	await expect(livinityd.client.backups.listBackups.query({repositoryId})).resolves.toHaveLength(2)
})
...
test('getRepositorySize() returns the size of a repository', async () => {
	...
	await expect(livinityd.client.backups.getRepositorySize.query({repositoryId})).resolves.toMatchObject({
		used: 0,
...
test('backups sets user notification if backups have not run in over 24 hours', async () => {
	...
	await expect(livinityd.client.notifications.get.query()).resolves.toHaveLength(0)
	...
	// Verify the share is no longer mounted
	// TODO: For some reason if we remove this line the test fails, look into why
	await expect(livinityd.client.files.list.query({path: backupNetworkSharePath})).rejects.toThrow('EHOSTDOWN')
	...
	await pRetry(
		() =>
			expect(livinityd.client.notifications.get.query()).resolves.toMatchObject([
	...
	await pRetry(() => expect(livinityd.client.notifications.get.query()).resolves.toHaveLength(0), {
		retries: 65,
		factor: 1,
	})
	...
	livinityd.instance.backups.backupInterval = 1000000
})

test('backup can be restored on the current Livinity install', async () => {
	// Create a file to mark the installation
	await livinityd.client.files.createDirectory.mutate({path: '/Home/original-livinity'})
	...
	await livinityd.client.files.trash.mutate({path: '/Home/original-livinity'})
	...
	await expect(livinityd.client.backups.restoreBackup.mutate({backupId: latestBackup.id})).rejects.toThrow(
		'[not-enough-space]',
	)
	getSystemDiskUsageSpy.mockRestore()

	// Now restore should succeed with normal disk usage
	await livinityd.client.backups.restoreBackup.mutate({backupId: latestBackup.id})

	// After restore (no reboot in tests), the restore marker should exist under /import
	const importFlagPath = `${livinityd.instance.dataDirectory}/import/${BACKUP_RESTORE_FIRST_START_FLAG}`
	...
	await expect(livinityd.client.backups.restoreStatus.query()).resolves.toMatchObject({
		running: false,
		progress: 100,
		error: false,
	})
	...
	expect(await fse.pathExists(`${livinityd.instance.dataDirectory}/home/original-livinity`)).toBe(false)
	...
	expect(await fse.pathExists(`${livinityd.instance.dataDirectory}/home/original-livinity`)).toBe(true)

	// Destroy the new livinity instance
	removeListener()
})

test('backup can be restored on a fresh livinity during setup', async () => {
	// Create a file to mark the installation
	await livinityd.client.files.createDirectory.mutate({path: '/Home/original-livinity'})
	...
	// Create a new livinity instance
	const newLivinityd = await createTestLivinityd({autoLogin: false, autoStart: true})

	// Simulate connecting the previous usb drive to it
	await fse.move(
		`${livinityd.instance.dataDirectory}/external/SanDisk`,
		`${newLivinityd.instance.dataDirectory}/external/SanDisk`,
	)

	// Connect to the USB backup repository
	const newRepositoryId = await newLivinityd.client.backups.connectToExistingRepository.mutate({
		path: `/External/SanDisk`,
		password: 'test-password',
	})
	...
	// Listen for restore progress events
	const restoreProgressEvents: any[] = []
	const removeListener = newLivinityd.instance.eventBus.on(
		'backups:restore-progress',
		(progress) => void restoreProgressEvents.push(JSON.parse(JSON.stringify(progress))),
	)

	// Get the latest backup and restore it
	const backups = await newLivinityd.client.backups.listBackups.query({repositoryId: newRepositoryId})
	const latestBackup = backups.at(-1)!
	expect(latestBackup).toBeDefined()
	await newLivinityd.client.backups.restoreBackup.mutate({backupId: latestBackup.id})

	// After restore (no reboot in tests), the restore marker should exist under /import
	const newImportFlagPath = `${newLivinityd.instance.dataDirectory}/import/${BACKUP_RESTORE_FIRST_START_FLAG}`
	...
	// Verify current progress is not running (final status is 100% after success)
	await expect(newLivinityd.client.backups.restoreStatus.query()).resolves.toMatchObject({
		running: false,
		progress: 100,
		error: false,
	})
	...
	await expect(newLivinityd.client.user.exists.query()).resolves.toBe(false)
	expect(await fse.pathExists(`${newLivinityd.instance.dataDirectory}/home/original-livinity`)).toBe(false)

	// Stop and start to simulate a reboot
	await newLivinityd.instance.stop()
	await newLivinityd.instance.start()

	// Check we now have a user and the marker file from the previous installation
	await expect(newLivinityd.client.user.exists.query()).resolves.toBe(true)
	expect(await fse.pathExists(`${newLivinityd.instance.dataDirectory}/home/original-livinity`)).toBe(true)

	// Destroy the new livinity instance
	removeListener()
	await newLivinityd.cleanup()
})
...
test('connectToExistingRepository() cleans up failed connection details', async () => {
	...
	await expect(
		newLivinityd.client.backups.connectToExistingRepository.mutate({
			path: `/External/SanDisk`,
	...
	await newLivinityd.client.backups.listBackups.query({repositoryId: newRepositoryId})
...
test('getRepositories() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.backups.getRepositories.query()).rejects.toThrow('Invalid token')
})

test('getRepositorySize() throws invalid error without auth token', async () => {
	await expect(
		livinityd.unauthenticatedClient.backups.getRepositorySize.query({repositoryId: 'test-repo'}),
	).rejects.toThrow('Invalid token')
})

test('createRepository() throws invalid error without auth token', async () => {
	await expect(
		livinityd.unauthenticatedClient.backups.createRepository.mutate({
			path: '/Network/test',
			password: 'test-password',
		}),
	).rejects.toThrow('Invalid token')
...
test('backup() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.backups.backup.mutate({repositoryId: 'test-repo'})).rejects.toThrow(
		'Invalid token',
	)
})

test('listBackups() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.backups.listBackups.query({repositoryId: 'test-repo'})).rejects.toThrow(
		'Invalid token',
	)
})

test('listAllBackups() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.backups.listAllBackups.query()).rejects.toThrow('Invalid token')
})

test('listBackupFiles() throws invalid error without auth token', async () => {
	await expect(
		livinityd.unauthenticatedClient.backups.listBackupFiles.query({backupId: 'test-repo:test-backup'}),
	).rejects.toThrow('Invalid token')
})

test('mountBackup() throws invalid error without auth token', async () => {
	await expect(
		livinityd.unauthenticatedClient.backups.mountBackup.mutate({backupId: 'test-repo:test-backup'}),
	).rejects.toThrow('Invalid token')
})

test('unmountBackup() throws invalid error without auth token', async () => {
	await expect(
		livinityd.unauthenticatedClient.backups.unmountBackup.mutate({directoryName: 'test-directory'}),
	).rejects.toThrow('Invalid token')
})

test('backupProgress() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.backups.backupProgress.query()).rejects.toThrow('Invalid token')
})

test('getIgnoredPaths() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.backups.getIgnoredPaths.query()).rejects.toThrow('Invalid token')
})

test('addIgnoredPath() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.backups.addIgnoredPath.mutate({path: '/Home/test'})).rejects.toThrow(
		'Invalid token',
	)
})

test('removeIgnoredPath() throws invalid error without auth token', async () => {
	await expect(livinityd.unauthenticatedClient.backups.removeIgnoredPath.mutate({path: '/Home/test'})).rejects.toThrow(
		'Invalid token',
	)
})
