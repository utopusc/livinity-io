...
describe(`backupProgress()`, () => {
	...
	test('returns backup progress during backup operation', async () => {
		...
		const backupPath = `${livinityd.instance.dataDirectory}/external/SanDisk`
		await fse.mkdir(backupPath, {recursive: false})

		// Create some test data to backup
		const testDataPath = `${livinityd.instance.dataDirectory}/home/backup-progress-test.txt`
		...
		// Start the backup
		const backupPromise = livinityd.client.backups.backup.mutate({repositoryId})
	...
	test.skip('handles multiple concurrent backups', async () => {
		...
		const backupPath1 = `${livinityd.instance.dataDirectory}/external/USB-1`
		const backupPath2 = `${livinityd.instance.dataDirectory}/external/USB-2`
		await fse.mkdir(backupPath1, {recursive: true})
		await fse.mkdir(backupPath2, {recursive: true})

		// Create test data
		const testDataPath = `${livinityd.instance.dataDirectory}/home/concurrent-backup-test.txt`
		...
		const backup1Promise = livinityd.client.backups.backup.mutate({repositoryId: repositoryId1})
		const backup2Promise = livinityd.client.backups.backup.mutate({repositoryId: repositoryId2})

		// Wait for both to start
		await delay(100)

		// Test we have two backup operations in progress
		const progressInProgress = await livinityd.client.backups.backupProgress.query()
