...
export default class User {
	...
	async syncSystemPassword() {
		try {
			...
			const hasLivinitySystemUser = userFile.split('\n').some((line) => line.startsWith('livinity:'))
			const hashedPassword = (await this.#store.get('user.hashedPassword')) || ''

			// Only attempt this if there's an livinity user and a password has been set
			if (hasLivinitySystemUser && hashedPassword.length > 0) {
				...
				if (supportsBcrypt) {
					...
					if (bcryptRegex.test(hashedPassword)) {
						...
						await $({input: `livinity:${systemPassword}`})`chpasswd --encrypted`
	...
	async register(name: string, password: string, language: string) {
		...
		await this.#livinityd.store.set('widgets', ['livinity:files-favorites', 'livinity:storage', 'livinity:system-stats'])
	...
	// Set custom accent color (hex format like #7c3aed, or null to use wallpaper color)
	async setAccentColor(accentColor: string | null) {
		if (accentColor === null) {
			return this.#store.delete('user.accentColor')
		}
		return this.#store.set('user.accentColor', accentColor)
	}

	// Get custom accent color
	async getAccentColor(): Promise<string | null> {
		return this.#store.get('user.accentColor') ?? null
	}
}
