...
export const systemWidgets = {
	storage: async function (livinityd: Livinityd) {
	...
	memory: async function (livinityd: Livinityd) {
	...
	'system-stats': async function (livinityd: Livinityd) {
		const [cpuUsage, diskUsage, memoryUsage] = await Promise.all([
			getCpuUsage(livinityd),
			getSystemDiskUsage(livinityd),
