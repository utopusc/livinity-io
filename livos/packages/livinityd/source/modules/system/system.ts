...
export async function getSystemDiskUsage(
	...
	return await getDiskUsageByPath(livinityd.dataDirectory)
}

export async function getDiskUsage(
	livinityd: Livinityd,
): Promise<{size: number; totalUsed: number; system: number; files: number; apps: DiskUsage[]}> {
	const {size, totalUsed} = await getSystemDiskUsage(livinityd)

	// Get app disk usage
	const apps = await Promise.all(
		livinityd.apps.instances.map(async (app) => ({
			id: app.id,
	...
	const filesTotalUsage = (
		await Promise.all(
			[
				livinityd.files.getBaseDirectory('/Home'),
				livinityd.files.getBaseDirectory('/Trash'),
				livinityd.files.thumbnails.thumbnailDirectory,
...
export async function getMemoryUsage(livinityd: Livinityd): Promise<{
	size: number
	totalUsed: number
	system: number
	apps: MemoryUsage[]
	...
	const apps = await Promise.all(
		livinityd.apps.instances.map(async (app) => {
			...
			try {
				...
				livinityd.logger.error(`Error getting memory`, error)
			}
			return {
				id: app.id,
				used: appUsed,
...
export async function getCpuUsage(livinityd: Livinityd): Promise<{
	threads: number
	totalUsed: number
	system: number
	apps: CpuUsage[]
	...
	const apps = await Promise.all(
		livinityd.apps.instances.map(async (app) => {
			...
			try {
				...
				livinityd.logger.error(`Error getting cpu`, error)
			}
			return {
				id: app.id,
				used: appUsed,
...
// as much Livinity stuff as possible ourselves before handing over to the OS.
...
export async function commitOsPartition(livinityd: Livinityd): Promise<boolean> {
	try {
		livinityd.logger.log('Committing OS partition...')
		await $`mender commit`
		livinityd.logger.log('Successfully commited to new OS partition.')
		return true
	} catch (error) {
		if (
			(error as ExecaError).stderr?.includes('level=error msg="Could not commit Artifact: There is nothing to commit"')
		) {
			livinityd.logger.log('No new OS partition to commit.')
			return true
		}
		livinityd.logger.error(`Failed to commit OS partition`, error)
		return false
	}
}
...
export async function detectDevice() {
	...
	if (model === 'U130120') device = 'Livinity Home (2023)'
	if (model === 'U130121') device = 'Livinity Home (2024)'
	if (model === 'U130122') device = 'Livinity Home (2025)'
	if (productName === 'Livinity Home') deviceId = model
	...
	// Blank out model and serial for non Livinity Home devices
	if (productName !== 'Livinity Home') {
		model = ''
		serial = ''
	}
...
export async function isLivOS() {
	return fse.exists('/LivOS')
}
...
export async function setupPiCpuGovernor(livinityd: Livinityd): Promise<void> {
	try {
		if (await isRaspberryPi()) {
			await setCpuGovernor('ondemand')
			livinityd.logger.log(`Set ondemand cpu governor`)
		}
	} catch (error) {
		livinityd.logger.error(`Failed to set ondemand cpu governor`, error)
	}
}
...
export async function connectToWiFiNetwork({ssid, password}: {ssid: string; password?: string}) {
	...
	try {
		...
		// conneciton in NetworkManager since it will be out of sync with livinityd.
		try {
		...
		// This is so livinityd retains ownership of the network connection management.
		// Otherwise if this fails nmcli will remember the connection and try to reconnect
		// which livinityd is not aware of.
		try {
			await deleteWifiConnections({inactiveOnly: true})
		} catch (error) {
			console.log(`Failed to cleanup WiFi connections: ${(error as Error).message}`)
...
export async function restoreWiFi(livinityd: Livinityd): Promise<void> {
	const wifiCredentials = await livinityd.store.get('settings.wifi')
	if (!wifiCredentials) return

	while (true) {
		livinityd.logger.log(`Attempting to restore WiFi connection to ${wifiCredentials.ssid}...`)
		try {
			await connectToWiFiNetwork(wifiCredentials)
			livinityd.logger.log(`WiFi connection restored!`)
			break
		} catch (error) {
			livinityd.logger.error(`Failed to restore WiFi connection, retrying in 1 minute`, error)
...
export function getIpAddresses(): string[] {
	...
	// - Livinity Home 2024: enp1s0, wlo1 (predictable naming)
...
export async function syncDns() {
	return await syncDnsQueue.add(async () => {
		...
		await $`systemctl restart livinity-dns-sync`
...
export async function waitForSystemTime(livinityd: Livinityd, timeout: number): Promise<void> {
	try {
		// Only run on Pi
		if (!(await isRaspberryPi())) return

		livinityd.logger.log('Checking if system time is synced before continuing...')
		let tries = 0
		while (tries < timeout) {
			tries++
			...
			if (isSynced) {
				livinityd.logger.log('System time is synced. Continuing...')
				return
			}
			livinityd.logger.log('System time is not currently synced, waiting...')
			await setTimeout(1000)
		}
		livinityd.logger.error('System time is not synced but timeout was reached. Continuing...')
	} catch (error) {
		livinityd.logger.error(`Failed to check system time`, error)
	}
}
