...
export default router({
	...
	connect: privateProcedure
		...
		.mutation(async ({input, ctx}) => {
			const previousWifiCredentials = await ctx.livinityd.store.get('settings.wifi')
			try {
				await connectToWiFiNetwork({ssid: input.ssid, password: input.password})
				await ctx.livinityd.store.set('settings.wifi', {ssid: input.ssid, password: input.password})
			} catch (error) {
				// Best effort attempt to restore previous credentials on failure
				if (previousWifiCredentials) {
					ctx.livinityd.logger.error(`Failed to connect to WiFi network, attempting to restore previous credentials...`)
					connectToWiFiNetwork(previousWifiCredentials).catch((error) => {
						ctx.livinityd.logger.error(`Failed to restore previous WiFi connection`, error)
					})
				}
	...
	disconnect: privateProcedure.mutation(async ({ctx}) => {
		...
		await ctx.livinityd.store.delete('settings.wifi')
