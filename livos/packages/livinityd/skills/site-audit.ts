...
import { researchPrompt, executePrompt, verifyPrompt, buildLearnedEntry } from '@nexus/core/lib';
...
3. Identify the key checks we should perform:
  ...
  const outputPath = path.join(paths.output, `audit-${Date.now()}.md`);
