...
// TODO: Integrate into livinity-apps CI?

const rootFolder = process.env.LIVINITY_ROOT ?? '/home/livinity/livinity'
...
for (const relativeAppStoreFolder of appStoreFolders) {
	...
	for (const relativeAppFolder of appFolders) {
		...
		const manifestFile = path.join(appFolder, 'livinity-app.yml')
		try {
