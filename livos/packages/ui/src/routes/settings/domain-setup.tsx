...
function StepDomain({
	...
	return (
		<div className='space-y-5'>
			<div>
				<h3 className='text-body-lg font-semibold text-text-primary'>Enter your domain</h3>
			...
			<div>
				<label className='mb-1.5 block text-caption font-medium text-text-secondary'>Domain name</label>
				<input
					type='text'
					value={domain}
			...
			<div className='flex justify-end pt-1'>
				<button
					...
					className='flex items-center gap-2 rounded-radius-md bg-violet-600 px-5 py-2.5 text-body font-medium text-white transition-all hover:bg-violet-500 disabled:opacity-40'
				>
...
function StepDnsRecords({
	...
	return (
		<div className='space-y-5'>
			<div>
				<h3 className='text-body-lg font-semibold text-text-primary'>Add DNS record</h3>
				<p className='mt-1 text-caption text-text-tertiary'>
				...
				<table className='w-full text-left text-caption'>
					<thead>
						<tr className='border-b border-border-default bg-surface-base'>
							<th className='px-4 py-2.5 font-medium text-text-secondary'>Type</th>
							<th className='px-4 py-2.5 font-medium text-text-secondary'>Name</th>
							<th className='px-4 py-2.5 font-medium text-text-secondary'>Value</th>
							<th className='px-4 py-2.5 font-medium text-text-secondary'>TTL</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							...
							<td className='px-4 py-3'>
								<span className='font-mono text-text-secondary'>{300}</span>
							</td>
						</tr>
					</tbody>
				</table>
			...
			<div className='flex justify-between pt-1'>
				...
				<button
					onClick={onNext}
					className='flex items-center gap-2 rounded-radius-md bg-violet-600 px-5 py-2.5 text-body font-medium text-white transition-all hover:bg-violet-500'
				>
...
function StepVerify({
	...
	return (
		<div className='space-y-5'>
			<div>
				<h3 className='text-body-lg font-semibold text-text-primary'>Verify DNS</h3>
				<p className='mt-1 text-caption text-text-tertiary'>
			...
			<div className='rounded-radius-md border border-border-default bg-surface-base p-4'>
				{verifyQuery.isFetching || checking ? (
					...
					<div className='space-y-3'>
						...
						{result && (
							<div className='rounded-radius-sm bg-surface-base px-3 py-2 text-caption'>
								<div className='flex items-center justify-between'>
									<span className='text-text-tertiary'>Current:</span>
									<span className='font-mono text-text-secondary'>{result.currentIp || 'No A record found'}</span>
								</div>
								<div className='mt-1 flex items-center justify-between'>
									<span className='text-text-tertiary'>Expected:</span>
									<span className='font-mono text-text-secondary'>{result.expected}</span>
								</div>
							</div>
						)}
...
function StepActivate({
	...
	return (
		<div className='space-y-5'>
			<div>
				<h3 className='text-body-lg font-semibold text-text-primary'>Activate HTTPS</h3>
				<p className='mt-1 text-caption text-text-tertiary'>
	...
	return (
		<div className='space-y-5'>
			...
			<div className='flex items-center justify-center gap-2 rounded-radius-md bg-surface-base px-4 py-3'>
				...
				<IconExternalLink size={14} className='text-text-tertiary' />
			</div>

			<div className='flex justify-center pt-2'>
				<button
					onClick={onClose}
					className='flex items-center gap-2 rounded-radius-md bg-violet-600 px-6 py-2.5 text-body font-medium text-white transition-all hover:bg-violet-500'
				>
					Done
				</button>
			</div>
		...
		<div className='space-y-4'>
			...
			<div className='flex gap-2'>
				<button
					...
					className='flex items-center gap-2 rounded-radius-md bg-surface-base px-4 py-2 text-caption text-text-secondary transition-all hover:bg-surface-2 hover:text-text-secondary'
				>
					<IconRefresh size={14} />
					Change domain
				</button>
