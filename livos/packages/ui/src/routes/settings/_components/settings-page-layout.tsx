...
interface SettingsPageLayoutProps {
	...
	backTo?: string
	/**
	 * v36 LivOS Design Port (Phase 124, additive opt-in).
	 * When true, suppresses the default title/description/back-button header bar
	 * so the consumer can render a <SettingsPageHeader/> instead. The Card wrapper
	 * + content area are still emitted. Default false → all existing settings
	 * pages render byte-identically.
	 */
	hideHeader?: boolean
}

export function SettingsPageLayout({title, description, children, backTo = '/settings', hideHeader = false}: SettingsPageLayoutProps) {
	...
	return (
		<div className='animate-in fade-in slide-in-from-right-4 duration-200 overflow-x-hidden'>
			<Card className={cn(isMobile ? '' : 'min-h-[500px]')}>
				{/* Header with back button — suppressed when consumer opts in to v36 SettingsPageHeader */}
				{!hideHeader && (
				...
				{/* Content */}
				<div className={cn('overflow-x-hidden', hideHeader ? '' : 'pt-6')}>{children}</div>
			</Card>
		</div>
	)
}
