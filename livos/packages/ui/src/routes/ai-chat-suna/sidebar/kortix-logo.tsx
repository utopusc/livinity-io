// Ported from Suna: components/sidebar/kortix-logo.tsx
// Substitutions: next/image -> <img>; next-themes -> removed (use className directly);
// Brand "Kortix" -> "Livinity"; SVG sourced from LivOS livinity-logo asset
...
export function KortixLogo({size = 24}: KortixLogoProps) {
  ...
  // Use the Livinity SVG logo inline so no external image dependency is needed
  return (
    <svg
      ...
      fill="none"
      aria-label="Livinity"
      className={`${mounted ? 'dark:invert' : ''} flex-shrink-0`}
