import {hostEnvironmentMap, LOADING_DASH, LivinityHostEnvironment, UNKNOWN} from '@/constants'
...
export function useDeviceInfo(): DeviceInfoT {
	...
	const livinityHostEnvironment: LivinityHostEnvironment | undefined = deviceInfoToHostEnvironment(deviceInfoQ.data)
	...
	return {
		...
		uiData: livinityHostEnvironment
			? {
					icon: hostEnvironmentMap[livinityHostEnvironment].icon,
...
function deviceInfoToHostEnvironment(deviceInfo?: DeviceInfo): LivinityHostEnvironment | undefined {
	if (!deviceInfo) {
		return undefined
	}

	if (deviceInfo.productName.toLowerCase().includes('livinity home')) {
		return 'livinity-home'
	}
