	...
	if (!app) {
		return (
			<div className='flex h-full items-center justify-center'>
				<p className='text-neutral-500'>App not found: {appId}</p>
			</div>
		)
	}
...
function TopHeaderWindow({app, childrenRight}: {app: RegistryApp; childrenRight: ReactNode}) {
	...
	return (
		<div className='space-y-5'>
			<button onClick={goBack} className='flex items-center gap-2 text-neutral-500 hover:text-neutral-900 transition-colors'>
			...
			<div data-testid='app-top' className='flex flex-col items-stretch gap-5 md:flex-row'>
				<div className='flex min-w-0 flex-1 items-center gap-2.5 md:gap-5'>
					<AppIcon src={app.icon} size={100} className='rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.08)] ring-1 ring-neutral-200/60 lg:rounded-3xl' />
					<div className='flex min-w-0 flex-col items-start gap-1.5 py-1 md:gap-2'>
						<h1 className='flex flex-wrap items-center gap-2 text-body-lg font-semibold leading-inter-trimmed text-neutral-900 md:text-heading-lg'>
							{app.name} {app.optimizedForLivinityHome && <Badge>{t('app.optimized-for-livinity-home')}</Badge>}
						</h1>
						<p className='line-clamp-2 w-full text-caption leading-tight text-neutral-500 md:line-clamp-1 md:text-body-lg'>
							{app.tagline}
						</p>
						<div className='flex-1' />
						<div className='text-caption text-neutral-500 delay-100 animate-in fade-in slide-in-from-right-2 fill-mode-both md:text-body-sm'>
							{app.developer}
						</div>
					</div>
				</div>
