...
export default function WebAppStreamWindow({webappId, windowId, urlOverride, titleOverride}: WebAppStreamWindowProps) {
	...
	// 4. VNC + agent hooks.
	// Phase 270-RFB: viewOnly=false — noVNC/RFB forwards real pointer +
	// keyboard + scroll events straight to x11vnc, which dispatches them via
	// XTest into THIS WebApp's own Xvfb display (one-Chrome-per-display, same
	// model as native app streams — native-app-stream-window.tsx also uses
	// viewOnly:false). This retires the entire client-side xdotool
	// input-forwarding layer (the old viewOnly:true + tRPC webapp.input.*
	// dispatch, incl. the 270-DRAG drag patch in 6f27ee5b): content
	// drag-and-drop, click, type, and scroll now work natively over RFB.
		...
		<div className='relative flex h-full w-full flex-col bg-surface-base'>
			<div className='relative flex-1 min-h-0 overflow-hidden bg-black'>
				{/* Phase 270-RFB — object-contain (parity with
				    ...
				    scaling against the rendered canvas; object-contain keeps
				    that mapping faithful (the canvas is the framebuffer
				    surface, no cropping) so clicks land where the user aims.
				    object-cover would crop the framebuffer and skew RFB's
				    coordinate translation. The container hosts noVNC's child
				    <canvas>; the [&_canvas] Tailwind selectors size it. */}
				<div
