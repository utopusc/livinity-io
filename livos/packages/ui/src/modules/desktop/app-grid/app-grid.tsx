...
export function AppGrid({
	...
	const {pageInnerRef, pages, appsPerRow, hasMeasurement} = usePager({apps})
	...
	const noRoom = hasMeasurement && apps.length > 0 && pages[0].apps.length === 0
	...
	return (
		<div className='flex h-full w-full flex-grow flex-col items-center'>
			<div className='relative flex w-full flex-grow justify-center overflow-hidden'>
				<div
					ref={scrollContainer}
					className='livinity-hide-scrollbar flex h-full w-full snap-x snap-mandatory overflow-hidden overflow-x-auto md:max-w-[var(--apps-max-w)]'
				>
