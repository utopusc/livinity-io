 ...
 *
 * ORDER MATTERS — this is the order the picker popover displays. The
 * Vitest case in `mention-adapter.test.ts` pins this order; any reorder
 * is a breaking change to the operator UAT script (Plan 200-08
 * checklist § @-mention picker).
 *
 * Entries 1-3 are Phase 200-C built-in Mastra tools. Entries 4-7 are
 * Luse computer-use tools (existing registrations on the backend agent).
 */
 ...
 *
 * Hook itself is exercised at runtime by Plan 200-05's composer test
 * + Plan 200-08's operator UAT (`@` → popover open → Enter to insert
 * `:tool[label]{name=id}` directive). The Vitest gate in
 * `mention-adapter.test.ts` only pins the static catalog content —
 * a render-time test would require AssistantRuntime context which is
 * intentionally out of scope for this isolated adapter unit.
 */
