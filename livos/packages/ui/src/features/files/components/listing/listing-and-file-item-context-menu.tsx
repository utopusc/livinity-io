...
export function ListingAndFileItemContextMenu({children, menuItems}: ListingAndFileItemContextMenuProps) {
	...
	if (isFileMenu) {
		...
		if (isBrowsingTrash) {
			...
			const canOpen = hasOneSelectedItem && !isUnmountedNetworkHost && !isDirectoryALivinityBackup(item.name)
			const canRename =
				hasOneSelectedItem && item.operations.includes('rename') && !isDirectoryALivinityBackup(item.name)
			...
			const canArchive = !(isViewingNetworkDevices || isViewingNetworkShares || isDirectoryALivinityBackup(item.name))
