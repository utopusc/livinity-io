...
export function SearchInput() {
	...
	return (
		<div
			className={cn(
				'relative flex items-center gap-1.5 h-8 px-2.5 rounded-xl bg-[color:var(--bg-2)] border border-transparent transition-all duration-300',
				isFocused && 'bg-[color:var(--bg)] border-line-strong shadow-sm',
			)}
		>
			{isFocused && (
				<BorderTrail
			...
			<TbSearch
				className='h-3.5 w-3.5 shrink-0 cursor-text text-[color:var(--fg-mute)]'
				strokeWidth={2.5}
				onClick={() => inputRef.current?.focus()}
			/>
			<Input
				className={cn(
					'h-full !border-none !bg-transparent !p-0 text-[13px] text-[color:var(--fg)] placeholder:text-[color:var(--fg-faint)] !outline-none !ring-0 transition-all duration-300 w-24 focus:w-36',
					{
						'w-36': query.length > 0,
					},
				)}
