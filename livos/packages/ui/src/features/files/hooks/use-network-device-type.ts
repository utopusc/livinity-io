...
/**
 * Service to detect the type of network device (Livinity or generic NAS)
 */

type NetworkDeviceType = 'livinity' | 'nas'

/**
 * Extract hostname from network path
 * @param path Network path like "/Network/livinity.local" or "/Network/192.168.1.100"
...
/**
 * Hook to detect and cache the type of network device (Livinity or generic NAS)
 * @param path Network path like "/Network/livinity.local"
...
export function useNetworkDeviceType(path: string) {
	...
	const optimisticDeviceType: NetworkDeviceType = hostname?.toLowerCase().includes('livinity') ? 'livinity' : 'nas'

	// NOTE: tRPC endpoint name kept as-is (backend API)
	const query = trpcReact.files.isServerAnLivinityDevice.useQuery(
		{address: hostname!},
		{
			// Cache for 1 hour
			gcTime: 60 * 60 * 1000,
	...
	if (!hostname) {
		...
		deviceType = query.data === true ? 'livinity' : 'nas'
	} else {
