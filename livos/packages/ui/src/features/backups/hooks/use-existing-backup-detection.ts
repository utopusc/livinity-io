...
export function useExistingBackupDetection(folder: string | undefined, repositories: BackupRepository[] | undefined) {
	...
	// Detect whether the selected folder contains an Livinity backup repository and
	// whether that repository is already configured on this Livinity.
	useEffect(() => {
		let cancelled = false
		async function detect() {
			...
			try {
				...
				// We check for both the repo path (eg. /External/USB-DISK/Data/Livinity Backup.backup) and the folder path (eg. /External/USB-DISK/Data)
