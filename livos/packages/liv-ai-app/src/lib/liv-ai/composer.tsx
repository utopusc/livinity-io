 ...
 *   composer.tsx (Plan 200-05 ship, D-201-21 reuse 1:1). Path remaps:
 ...
 *     @/components/assistant-ui/composer-trigger-popover (unchanged — ported here)
 *     livos UI shadcn button path             → @/components/ui/button
 *
 * Button variants remapped to subapp's shadcn registry:
 *   variant="primary"   → variant="default"
 *   size="icon-only"    → size="icon"
 *
 * Rewrites the Phase 199-05 scaffold into the canonical surface
...
/**
 * Canonical Liv AI composer. Mounted via the Thread `composerSlot` prop
 * by assistant.tsx — runtime preserves text + focus across the
 * empty→chat transition (RESEARCH Pitfall 7).
 */
        ...
        <div className="flex w-full flex-col gap-2 rounded-3xl border bg-background p-2.5 focus-within:border-ring/75 focus-within:ring-2 focus-within:ring-ring/20">
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1">
               ...
               * LivAiModelPicker has no internal collapse CSS, so the
               * wrapper layer applies `group-data-[empty=...]/composer:max-w-*`
               * against parent <ComposerPrimitive.Root data-empty="…">.
               */}
