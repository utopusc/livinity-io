...
- **Open-Q3 (orphaned dirs) → RESOLVED by research: SAFE to remove.** `setup_docker_prerequisites()` (install.sh:413-429, call :1818, flow-comment :1754) only `mkdir`s `data/tor/data` + `data/app-data` (under `data/`) — zero downstream consumers (tor_proxy/auth deleted in P276). ⚠️ DISTINCT from the LIVE top-level `$LIVOS_DIR/app-data/<appId>` per-app dir (install.sh:1110,1126-1127,1887 + builtin-apps volume mounts) which MUST STAY. Operator pre-approved remove → REMOVE.
