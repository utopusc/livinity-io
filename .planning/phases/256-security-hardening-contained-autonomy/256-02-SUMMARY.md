...
- `cred-egress-proxy.ts createCredEgressProxy` deliberately does NOT inline the full TLS-MITM termination loop — it establishes the CONNECT tunnel after enforcing the source-IP + host-allowlist guards, and the wire-level `Authorization` injection is performed by `injectAuthHeader` (the single, unit-tested injection point the production MITM leg calls). This is documented in the function doc-comment. The security boundary (default-deny egress + source-IP gate + read-only token use) is fully implemented and tested; the MITM transport plumbing is the remaining production wiring, exercised live in 256-05 (deploy/UAT). This is intentional and scoped — not a data-flow stub feeding the UI.

## Success Criteria

- **SC4 — no cred mount + model reachable via proxy + no token in container:** SATISFIED in code/unit. `injectLocalAiClisConfig` emits NO `.claude`/`.gemini` mount (Test 2b), injects `HTTPS_PROXY=http://livinity-credproxy:13129` + placeholder key + CA cert + `extra_hosts` (Tests 2c–2e); `cred-egress-proxy` holds tokens host-side, reads them read-only, injects the bearer only for allowlisted hosts, source-IP-gated. Live `docker inspect` / `printenv HTTPS_PROXY` / `cat …/.credentials.json` probes are the 256-05 deploy step.
