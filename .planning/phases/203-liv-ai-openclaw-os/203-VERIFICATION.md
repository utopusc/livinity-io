...
| Plan 203-12 deferred #3 | Liv AI claw-plugin not in loaded plugins | **RESOLVED** — Fix-A (`26444ce0`) + Fix-B (`f69525cd`) + Fix-C (`08511784` + `2b2c9f73`) all shipped 2026-05-24. Plugin in `installs.json` + 8-plugin boot line; all 20+ tools registered + callable; ZERO contracts.tools rejection lines; `<title>Liv AI</title>` served end-to-end via Caddy on bruce.livinity.io/liv-ai-app/openclawos. See `203-HOTFIX-205-PLUGIN-LOAD.md` for full forensics. |
