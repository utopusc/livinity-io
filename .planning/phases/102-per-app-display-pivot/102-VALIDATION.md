...
| 102-08-01 | 08 | 3 | D-102-CLOSE-LIFECYCLE | — | close order: Chrome SIGTERM→SIGKILL→x11vnc→Xvfb→rm /tmp→release port+display | unit | `pnpm --filter @livos/livinityd test:run webapps/window-manager.test.ts` | ✅ (extend) | ❌ red |
| 102-08-02 | 08 | 3 | D-102-CLOSE-LIFECYCLE | — | idempotent close (re-call no-op) | unit | same as 102-08-01 | ✅ (extend) | ❌ red |
