...
| 102-06-02 | 06 | 2 | D-102-LUSE-DISPLAY-SCOPING | — | agent-prompt-builder injects "Active Display Context" snippet with :N + bounds | unit | `pnpm --filter @livos/livinityd test:run ai/agent-prompt-builder.test.ts` | ✅ (extend) | ✅ green |
| 102-06-03 | 06 | 2 | D-102-LUSE-DISPLAY-SCOPING | T-102-06 | mcp/server.ts reads LUSE_TARGET_DISPLAY env, scopes X11 ops to that display | unit | `pnpm --filter @livos/livinityd test:run computer-use/mcp/server.test.ts` | ❌ create or extend | ✅ green |
| 102-07-01 | 07 | 3 | D-102-MASTER-LOGIN-UI | T-102-07 (admin gate) | adminProcedure on chromeMaster.startLogin mutation | unit | `pnpm --filter @livos/livinityd test:run chrome-master/master-login-routes.test.ts` | ✅ created | ❌ red |
...
| 102-08-01 | 08 | 3 | D-102-CLOSE-LIFECYCLE | — | close order: Chrome SIGTERM→SIGKILL→x11vnc→Xvfb→rm /tmp→release port+display | unit | `pnpm --filter @livos/livinityd test:run webapps/window-manager.test.ts` | ✅ (extend) | ✅ green |
| 102-08-02 | 08 | 3 | D-102-CLOSE-LIFECYCLE | — | idempotent close (re-call no-op) | unit | same as 102-08-01 | ✅ (extend) | ✅ green |
