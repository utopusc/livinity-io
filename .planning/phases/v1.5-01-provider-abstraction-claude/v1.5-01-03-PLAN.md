...
3. `chatStream(options: ProviderChatOptions): ProviderStreamResult`
   ...
   - KEEP the existing retry-within-stream logic from Brain.chatStream() -- it handles Gemini-specific stream errors
...
The implementation should be nearly identical to the current brain.ts Gemini code, just reorganized into the AIProvider interface shape with message format conversion delegated to the normalization layer.
  ...
  <verify>
`cd nexus && npx tsc --noEmit --pretty 2>&1 | head -20` -- no TypeScript errors. `grep 'implements AIProvider' nexus/packages/core/src/providers/gemini.ts` confirms interface implementation. `grep 'GoogleGenerativeAI' nexus/packages/core/src/providers/gemini.ts` confirms Gemini SDK import.
  </verify>
  <done>
GeminiProvider implements AIProvider with all methods extracted from Brain class. Uses prepareForProvider('gemini') for message conversion. Preserves existing retry logic for stream errors. Reads API key from Redis `livos:config:gemini_api_key` with env fallback.
  </done>
</task>
...
**chatStream(options) method -- WITH fallback:**
...
- If the stream has already started yielding chunks, do NOT fall back (partial response would be confusing)
- Implementation approach:
  ...
  4. If error after first yield, re-throw (let caller handle)

IMPORTANT: The chatStream fallback is the most complex part. Here's the pattern:
    ...
    for (const providerId of self.fallbackOrder) {
      ...
      try {
        ...
        if (self.isFallbackableError(err)) {
          logger.warn(`ProviderManager: ${providerId} stream failed, trying next`, {
            error: err.message,
          });
          continue;
        }
...
- Allow dynamic reordering (for when user changes primary provider via settings)
  ...
  <verify>
`cd nexus && npx tsc --noEmit --pretty 2>&1 | head -20` -- no TypeScript errors. `grep 'isFallbackableError' nexus/packages/core/src/providers/manager.ts` confirms fallback error detection. `grep -c 'fallbackOrder' nexus/packages/core/src/providers/manager.ts` shows multiple references to fallback chain.
  </verify>
  <done>
ProviderManager registers Claude (primary) and Gemini (fallback). chat() and chatStream() try Claude first, fall back to Gemini on 429/503/timeout. chatStream() handles the streaming fallback correctly (only falls back before first chunk). isFallbackableError() distinguishes retryable errors from non-retryable ones.
  </done>
</task>
...
5. GeminiProvider behavior is identical to the old Brain Gemini implementation
6. Brain.chat() and chatStream() call normalizeMessages() to convert 'model' -> 'assistant'
7. providers/index.ts exports: types, normalize, ClaudeProvider, GeminiProvider, ProviderManager
8. `grep -r 'GoogleGenerativeAI' nexus/packages/core/src/brain.ts` returns empty (no Gemini code in Brain)
</verification>
