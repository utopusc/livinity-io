...
- **One-line verification harness (P97-03)**: `bash scripts/verify-sacred-sha.sh` (POSIX) or `node scripts/verify-sacred-sha.cjs` (Windows dev). PASS prints `[verify-sacred-sha] PASS: liv/packages/core/src/sdk-agent-runner.ts = f3538e1d811992b782a9bb057d1b7f0a0189f95f` and exits 0. FAIL exits 1 with locked vs current SHA diff and a reminder pointing back at this CONTEXT.md. Every `[verify-sacred]` checkbox in this phase invokes one of these.
