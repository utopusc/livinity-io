...
**Part C: Actually the tRPC routes from Part A are NOT needed** since skills-panel.tsx uses direct fetch through `skillFetch` helper which hits the Nexus API directly (via proxy). The existing pattern doesn't use tRPC for skills. However, the tRPC routes listed in Part A could still be useful for other consumers. For simplicity and consistency with the existing pattern, SKIP the tRPC routes for registry management and rely on the existing `skillFetch` pattern. Remove the tRPC route additions from Part A of the action.

HOWEVER: The `refreshCatalog` and `listRegistries` tRPC routes are still useful for potential future consumers. Add them anyway as they follow the existing proxy pattern used for other settings.
  </action>
  <verify>
