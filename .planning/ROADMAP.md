...
### Phase 198: Liv AI v2 — assistant-ui + Generative UI + tool-ui + Mastra Production Polish — 🔴 PLANNED 2026-05-23
...
Plans:
- [ ] 198-01-PLAN.md — Backend: `@mastra/ai-sdk` install + `chatRoute({path:'/chat/livAi'})` Express mount with JWT gate + zod-validated request body + integration test for SSE response shape (Wave 1, no deps)
- [ ] 198-02-PLAN.md — Frontend: install `@assistant-ui/react @assistant-ui/react-ai-sdk @assistant-ui/react-markdown` + delete `livos/packages/ui/src/features/liv-ai/*` (P197 bespoke) + scaffold Thread via `npx assistant-ui@latest add thread` + new `assistant.tsx` (AssistantRuntimeProvider + Thread) + rewire `liv-ai-content.tsx` (Wave 1, file-disjoint with 198-01)
- [ ] 198-03-PLAN.md — tool-ui primitives copy-paste (Image Gallery, Geo Map, Item Carousel, Weather, Data Table, Chart, Link Preview, Approval Card, Code Block, Code Diff, Sources, Tool Fallback) + tool-renderers.tsx registering `makeAssistantToolUI` for 10 named tools (`web_search`, `image_search`, `places_search`, `weather`, `map`, `data_query`, `chart`, `code_diff`, `link_preview`, `luse_computer_screenshot`, `luse_list_windows`, + fallback) + add `react-leaflet leaflet recharts` deps + unit tests per renderer (Wave 2, depends 198-02)
- [ ] 198-04-PLAN.md — HITL Approval Card flow per `assistant-ui/mastra-hitl` reference — Approval Card renders inline for the 6 `destructiveToolNames` (P197-02 N-01 lock import) + Approve/Reject buttons wire to existing `mastra.agent.approve` tRPC mutation (P197-05 KEEP) + integration test (mock chatRoute yields destructive tool-call chunk, assert Approval Card renders + click Approve fires mutation) (Wave 2, depends 198-02 + 198-03)
- [ ] 198-05-PLAN.md — ThreadList sidebar primitive + `ExternalStoreThreadListAdapter` wired to `mastra.agent.threads.list` query + `mastra.agent.threads.delete` mutation (P197-05 KEEP) + "New conversation" button + thread title fallback "Untitled · {timestamp}" (title-generation adapter deferred to P199) + tests (Wave 3, depends 198-02)
- [ ] 198-06-PLAN.md — Composer power features: 4 slash commands (`/help`, `/clear`, `/screenshot`, `/search`) + 4 suggested-prompt chips on empty thread + attachment adapter for image upload (multimodal Grok context) + unit tests for command parser + chip click handler (Wave 3, depends 198-02)
- [ ] 198-07-PLAN.md — Empty-state + onboarding tour + theming (Tailwind dark mode + livos-design-tokens) + DevTools panel gated behind `import.meta.env.DEV` (build-time tree-shake out of prod) + accessibility (ARIA labels, Cmd+Enter send, Esc cancel) + visual smoke test (Wave 3, depends 198-02)
- [ ] 198-08-PLAN.md — Mini PC deploy via `bash /opt/livos/update.sh` + sudoers/bruce-ownership patch + restart livos+liv-* + operator-walked UAT (9-step browser walk per CONTEXT.md acceptance envelope) + mark P197 tRPC `mastra.agent.*` namespace as deprecated header comment + STATE/ROADMAP flip to CODE-COMPLETE+LIVE + sacred SHA final verify (Wave 4, depends 198-01..07; human-gated)
