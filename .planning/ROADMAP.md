...
### Phase 226: Caddy `/liv` reverse proxy + iframe headers — ❌ BLOCKED (2/3 plans shipped, Plan 03 Rule 4 architectural)
...
- [~] 226-03-PLAN.md — Mini PC deploy + 3-run idempotency + 6 SC evidence captures. ❌ **BLOCKED at preflight** (2026-05-27). Rule 4 architectural STOP: Mini PC `/etc/caddy/Caddyfile` is dynamically generated by `livos/packages/livinityd/source/modules/domain/caddy.ts` (Phase 86 / Phase 218 lineage); the live form is `http://bruce.livinity.io {` (tunnel-mode scheme prefix from caddy.ts:342), which the installer's regex `^[[:space:]]*bruce\.livinity\.io[[:space:]]*\{` misses (Finding 1, Rule 1 bug). Even if regex widened, in-place `import` line edits would be wiped on next livinityd regen (Finding 2). 2/6 SCs PASS (SC-05 sacred SHA + SC-06 Caddyfile ownership); 4/6 NOT-EXERCISED. No live Mini PC changes; all 6 services unaffected; sacred SHA `f3538e1d811992b782a9bb057d1b7f0a0189f95f` byte-identical. **Critical:** commits `bef03544`+`1e56b8c9` on master mean next `bash /opt/livos/update.sh` on Mini PC will abort at Step 4.7 installer fail (services not regressed but update.sh unrunnable). **Three Options for closure (operator decision):** A (RECOMMENDED) — Phase 226-04 patches caddy.ts to emit `/liv` handler inline (~50 LOC + 1 test); B — same + drop snippet file; C — caddy.ts emits both `import` lines + installer still lays snippet. Mitigations to unblock update.sh: M1 ship 226-04, M2 revert `bef03544`+`1e56b8c9`, M3 patch installer skip-with-warn. DEPLOY-LOG (287 lines) + SUMMARY at `.planning/phases/226-caddy-liv-proxy-iframe-headers/226-03-{DEPLOY-LOG,SUMMARY}.md`.
