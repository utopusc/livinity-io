...
**Status:** CODE-COMPLETE 2026-05-12 + **UAT WALKED 2026-05-12 → PARTIAL PASS**. Plans 105-01/02/03 ✅ shipped; 105-04 Task 1 ✅ scaffold + Task 2 ✅ operator walk DONE (autonomous walk per user "professionalce" direction). Combined static tests: 117 + 18 + 24 = **159 PASS, 0 FAIL**. Sacred SHA preserved 10/10 commits (290885bc..733ba90f). **UAT result:** TLS pipeline + deploy-livinityd 1:1 update.sh port verified working; LE green-padlock cert + `<title>Livinity</title>` HTML proven during livinityd alive-window. GO-1/2/3 PARTIAL (livinityd flaps every ~12-15s from upstream gaps); GO-4 PASS (sacred SHA preserved on VPS); GO-5 SKIPPED. **10 bugs discovered** during UAT: 5 in Phase 105 deploy-livinityd scope (queued as Plan 105-05 back-port), 5 upstream bootstrap-layer (auth-server image / mender / samba / chrome / bruce-user — deferred to v34). See `UAT-CHECKLIST.md` for full bug list + hot-fix commands.
...
- [ ] 105-05-PLAN.md (Wave 4, autonomous, deps: 105-04 UAT signoff) — **HOTFIX BACK-PORT** of 5 deploy-livinityd bugs discovered during 105-04 live UAT: (1) pnpm 11 ERR_PNPM_IGNORED_BUILDS → add `--config.dangerously-allow-all-builds=true` to both pnpm install paths in `_dld_build_packages`; (2) gallery cache `find /nonexistent | head -1` pipefail kill → append `|| true` to assignment in `_dld_update_gallery_cache`; (3) cli.ts not executable after rsync → `chmod +x source/cli.ts` in `_dld_fix_permissions` (extends Mini PC behavior); (4) Caddyfile mode 0600 root-only → write with `0644` or chown caddy:caddy in `_dld_update_caddy_to_livinityd`; (5) livos.service ExecStart wrong invocation → replace `pnpm --filter livinityd start` with `/usr/bin/npx tsx /opt/livos/packages/livinityd/source/cli.ts --data-directory /opt/livos/data --port 8080` (matches Mini PC `livos/install.sh:1332`). Test extension: +5 positive-greps verifying each fix. Re-test on mainserver after merge to confirm GO-1/2/3 promote from PARTIAL to PASS (within bootstrap-layer constraints).
