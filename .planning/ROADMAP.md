...
Plans:
- [x] 256-01-PLAN.md (Wave 1) — WS-A Contained Autonomy: bubblewrap sandbox + cred-scrub for `shell`, path-allowlist for `files`, per-session git snapshot, tinyproxy egress allowlist (LIVOS-002 / SC1-3). **DONE** 2026-06-03.
- [x] 256-02-PLAN.md (Wave 2, deps 01) — WS-B Credential Egress Proxy: host-side credential-injecting proxy replaces the `~/.claude`/`~/.gemini` RW bind mounts (verified apps get OAuth at the wire); PLUS the per-app metered virtual-key path for UNVERIFIED/community apps via the broker (budget+model allowlist, revocable) (LIVOS-001 / SC4 + SC4b). **DONE** 2026-06-03 (4 commits `45c11761`/`232c19b4`/`755f36ef`/`bd21d9a6`, 31 WS-B unit cases green).
...
**Plans:** 0 plans (research-only phase — deliverable produced)

**Research status:** ✅ COMPLETE 2026-06-09. Deliverable `SECURITY-RESEARCH-PASS-3.md` written to repo root (18 new findings LIVOS-041..058: 2 Critical / 7 High / 5 Medium / 4 Low; WS-B fast-follow table; WS-C industry recs; proposed Phase 262 scope). Method: 52-agent workflow (per-surface finders → adversarial per-finding verifiers → synthesis). Top risks: LIVOS-041 (unauth `/liv-login` → operator-credentialed agent → host root), LIVOS-042 (native `installV37` non-admin client-manifest RCE as bruce), LIVOS-043 (`bruce ALL=(ALL) NOPASSWD:ALL` live-confirmed amplifier). Several feared regressions refuted (no silent unsandboxed bwrap fallback; fail2ban router clean; no session-revoke IDOR; timedatectl injection closed). Server5 documented-only, NOT contacted (hard rule honored).

Plans:
- [x] Research complete → `SECURITY-RESEARCH-PASS-3.md` (repo root). Next: `/gsd-plan-phase 261` (or hand report to new Phase 262 — report proposes a 6-workstream scope).

---

### Phase 262: Security Hardening Pass 3 — close the /liv front door + native-installer lockdown + sudoers/authz/secrets remediation

**Goal:** Remediate the 18 findings (`LIVOS-041..058`) surfaced by Phase 261's research (`SECURITY-RESEARCH-PASS-3.md`). Six severity-ordered workstreams: **WS1 (Critical)** close the `/liv` AionUi unauthenticated front door — `forward_auth` over the whole `/liv` family + a fail-closed apex gate + gate the `/liv-login` Express handler + replace the spoofable Referer-only `/api` matcher with a path-prefix matcher + bind AionUi `:3020` to loopback + wire jti revocation/active-user re-check into `/auth/verify`, `/__livos_sso`, `/__livos_auth` (LIVOS-041/047/049/053/054 + revocation gap); **WS2 (Critical)** lock down the native installer — make `installV37`/`uninstallV37` `adminProcedure` + re-fetch the manifest server-side from the trusted catalog (discard client `manifest`), validate `aptPackages`/URLs/apt-key fingerprints, make download checksums mandatory, re-validate every redirect (LIVOS-042/044/045/055); **WS3 (High)** delete the blanket `bruce ALL=(ALL) NOPASSWD:ALL` sudoers drop-in in `deploy-livinityd.sh`, rely on the scoped Phase-192 `Cmnd_Alias` fragment (LIVOS-043); **WS4 (High/Medium)** re-classify the new Settings power/storage/share backends as `adminProcedure`, validate `deviceId` + CIFS inputs (block injection/SSRF), add a per-app secret + interface-bind to the cred-egress proxy (LIVOS-046/048/050/051/056); **WS5 (Medium/Low)** finish the credential-store DEK migration to git/stack/backup siblings + legacy fallback, thread `requiresLocalAiClis`/`neverPublic` into the builtin/native/platform manifest writers + OR into `buildPublicForbiddenSignals`, replace the bwrap `--version` gate with a runtime userns probe (no silent unsandboxed fallback), add the `routes.ts:517` `legacySingleUser` parity guard, move the Kopia backup password off cleartext + derive the backup-vault key from a dedicated secret (LIVOS-052/052b/057/058); **WS6 (no-code)** operator out-of-band confirmations — live Server5 secret rotation (Server5 OFF-LIMITS, document only), `/etc/sudoers.d/99-bruce` live removal, `bruce` disk-group/polkit posture, `install.sh User=root` default. **CODE ONLY — NO DEPLOY** (Mini PC only; operator deploys via `update.sh` + does the live walk, per project convention). Server4/Server5 off-limits.
**Requirements**: remediation of confirmed findings LIVOS-041..058 (see `SECURITY-RESEARCH-PASS-3.md` for full exploit sketches + recommendations + success criteria)
**Depends on:** Phase 261 (research deliverable); inputs `SECURITY-RESEARCH-PASS-3.md` + `SECURITY-AUDIT.md` (repo root)
**Plans:** 5 plans (2 waves)

Plans:
- [x] 262-01-PLAN.md — WS1 close the /liv unauthenticated front door: forward_auth over the whole /liv family + apex fail-closed gate + verifySession on /liv-login + path-prefix /api matcher + jti revocation re-check (LIVOS-041/047/049/053/054 + revocation gap) [wave 1] — ✅ CODE-COMPLETE 2026-06-09, commits `b3267493`+`8fb779e0`+`8eb76653`+`782f1a96`, SUMMARY in phase dir (checkbox reconciled by 262-05 from on-disk SUMMARY)
...
- [x] 262-03-PLAN.md — WS3 remove the blanket bruce NOPASSWD:ALL sudoers grant + scoped LIVINITYD_FAIL2BAN alias + sudo -n fail2ban client (LIVOS-043) [wave 1] — ✅ CODE-COMPLETE 2026-06-09, commits `9cab3386`+`12d0f225`+`d674c099`, SUMMARY in phase dir (checkbox reconciled by 262-05 from on-disk SUMMARY)
- [x] 262-04-PLAN.md — WS4 adminProcedure for power/storage/share backends + deviceId/CIFS validation + cred-egress per-app token + gateway bind (LIVOS-046/048/050/051/056) [wave 1] — ✅ CODE-COMPLETE 2026-06-09, commits `f21ecbaa`+`54dc92c3`+`d7642873`, SUMMARY in phase dir (checkbox reconciled by 262-05 from on-disk SUMMARY)
...
- [x] 263-03-PLAN.md — L-062 container/host RBAC: ssh-sessions pattern on terminal/docker-exec/docker-logs/docker-file + userOwnsContainer ownership (Wave 3) ✅ CODE-COMPLETE 2026-06-10 (commits d49adf9b/6c86c0d8/34d1f35b/f8b742f2; 31/31 vitest green; userOwnsContainer fail-closed predicate + admin-OR-ownership re-verify at each WS handler boundary → non-owner member ws.close(4403), host-shell branch admin-only, GET/POST docker-file endpoints 403 non-owner; zero new tsc errors (320 baseline); live wscat/curl matrix deferred to 263-06)
...
- [x] 267-03-PLAN.md — Auto-appear: debounced `liv-assistant` restart on auth success → agent Failed→ready in `/api/agents` + dialog "ready" state + Mini-PC UAT [wave 3, depends_on 267-01/02] — ✅ CODE-COMPLETE 2026-06-14, commits `87e69678`+`8f5863e9`+`e71b55dd`, SUMMARY in phase dir; NEW `agent-refresh.ts` `scheduleAgentRefresh` = MODULE-level trailing-edge debounce (default 4s, burst→ONE restart) running best-effort argv `sudo -n systemctl restart liv-assistant` (existing NOPASSWD, no new grant) + SET `liv:cli:agent-refresh` restarting→done + :3020/api/agents probe — never throws, never invalidates the recorded auth; router fires it ONLY on auth/setApiKey `ok` + `cliInstaller.agentRefreshStatus` query; dialog `ready` state polls it → "Applying…"→"ready — open Liv AI" + Open-Liv-AI button w/ 12s graceful timeout. vitest 42/42 (agent-refresh 6 + router 36), common.test 18/18, onboarding 9/9, tsc baseline-identical (320, zero net new), `pnpm --filter ui build` exit 0. **Task 4 LIVE-BOX UAT (Failed→ready flip on a real AionUi restart) DEFERRED to operator** — CODE ONLY, this run can't touch the Mini PC. Operator deploys via update.sh + clear PWA SW
- [x] 267-04-PLAN.md — CLI/agent brand logos (`agent-logos.tsx` + `public/agent-logos/*.svg`) in the models picker + Local Agents panel, monogram fallback [wave 3] — ✅ CODE-COMPLETE 2026-06-14, commits `dec54271`+`875eacba`, SUMMARY in phase dir; `AGENT_LOGOS` dual-keyed (AionUi backend + CliName) + `<AgentLogo>` (static brand `<img>` or deterministic monogram, never a broken image) + 10 vetted brand SVGs; wired into model-picker (Grok keeps lucide via `fallbackIcon`) + Local Agents panel (`renderIcon` brand img w/ onerror→monogram). vite build exit 0, all 10 SVGs in dist/; agent-logos.test 10/10, model-picker 7/7. CODE ONLY — operator deploys via update.sh

### Phase 268: Interactive CLI auth (paste-back / device login) + CLI uninstall

**Goal:** Extend the Phase 267 no-terminal auth dialog so the CLIs that actually use a browser/device OAuth flow get a REAL guided flow instead of falling back to an API-key field — and add CLI uninstall. Today 14 of 20 SUPPORTED_CLIS are classified `apikey` in `auth-methods.ts` even though several carry a `loginArgv` for a real flow (`claude-code` → `claude setup-token`, `codex` → `codex login --device-auth`, plus `gemini`/`openclaw` `onboard`/`hermes` `setup --portal`). The operator's ask, verbatim: the CLI gives you a code/link, you authorize in the browser, then paste the resulting code BACK to the CLI. Today `authCli` only READS stdout — it never writes stdin — so a paste-back flow is impossible. (1) **Reclassify** those CLIs from `apikey` to their real branch (`device`, or a NEW interactive/paste-back branch) after researching each CLI's exact auth subcommand and whether it is device-poll vs paste-back vs localhost-redirect. (2) **Backend:** keep the spawned login process ALIVE after surfacing the URL/prompt and add `cliInstaller.sendAuthInput({name, code})` — a tRPC mutation that writes the pasted code to the running CLI's stdin (whitelist-guarded, D-239-07 name-only RCE boundary preserved, the code never logged), with a process registry + timeout/cleanup so a stranded login can't leak a live root-less child. (3) **Frontend:** a paste-back branch in `cli-auth-dialog.tsx` — show URL + explicit-click Open-link, then a "paste the code from the browser here" field → `sendAuthInput` → poll the existing `liv:cli:auth:<name>` completion key → success; keep 267's `device`/`apikey` branches and the debounced agent-refresh intact. (4) **CLI uninstall:** backend `uninstall({name})` that removes the locally-installed CLI per how it was installed (npm/apt/binary — researched per install method), a `cliInstaller.uninstall` tRPC mutation (adminProcedure, whitelist-guarded, with a UI confirm), and an Uninstall/Remove affordance on installed rows in the Local Agents panel + `cli-auth-dialog.tsx`, firing the same debounced agent-refresh so the removed agent disappears from `/api/agents`.
...
Plans:
- [x] 268-01-PLAN.md — backend paste-back foundation: 'paste-back' AuthBranch + claude bare loginArgv + live-child registry + sendAuthInput (TDD) — ✅ **SHIPPED 2026-06-14** (4 commits, TDD test→feat ×2: `09dcbca6`+`7ab566ab` Task 1 — `'paste-back'` AuthBranch + claude-code `loginArgv`/`CLI_AUTH_COMMANDS` → bare `['claude', []]` [setup-token's localhost callback fails headless; bare login prompts `Paste code here if prompted`], branch stays apikey [orthogonal capability], drift-lock still 20; `e9be43df`+`22400507` Task 2 — `liveAuths` Map + `registerLiveAuth` [single-in-flight SIGKILL + 300s `.unref()` timeout + natural-exit cleanup, called post-spawn in authCli] + exported `sendAuthInput({name,code})` [whitelist guard FIRST D-239-07, CR/LF-stripped+4096-capped code to child.stdin as DATA never argv, never logs the code, `{ok:false}` no-throw when no live login] + `_resetLiveAuthsForTests` + barrel re-exports). Verify: cli-installer suite **106/106** [no 267 regression], tsc 320 = baseline zero delta. **CODE ONLY — NOT DEPLOYED** (livinityd=tsx, no build; backend seam only — no route/transport/UI; 268-03 wires the tRPC route, 268-04 the dialog). SUMMARY: `.planning/phases/268-.../268-01-SUMMARY.md`.
- [x] 268-02-PLAN.md — CLI uninstall backend: CLI_UNINSTALL map + uninstallCli (npm/rm/pip, static-paths-only, aion-cli refused) (TDD) — ✅ **SHIPPED 2026-06-14** (2 commits, TDD test→feat: `1d2ef43a` RED + `c7f46b57` GREEN). NEW `cli-uninstall.ts` — drift-locked `CLI_UNINSTALL` map (20 keys, one per SUPPORTED_CLIS, mirrors `CLI_AUTH_METHODS`) classifying each CLI by install method: **npm-global** (`npm uninstall -g --prefix ~/.npm-global <pkg>`, argv-array no shell) gemini/codex/qwen-code/augment/github-copilot/codebuddy/qoder-cli; **rm-bin** (`fs.rm` curl bin + config dir) claude-code/opencode/goose/factory-droid/cursor-agent/kimi-cli[2 bins]/mistral-vibe/hermes-agent/kiro; **pip** (`pip3 uninstall -y nanobot-ai`) nanobot; **rm-paths** (static build/shim paths) snow-cli[3 paths] + openclaw[absolute `/opt/livos/bin/openclaw`]; **none** aion-cli refused. `uninstallCli({name})` — `SUPPORTED_CLIS_SET.has` is the FIRST statement (D-239-07; pkg/paths static-map-only, never request strings), deletes the 267 api-key file (`WRITE_TARGETS` relPath) + config dirs best-effort, snow-cli static-paths-only [E-5 — NEVER `command -v snow`, Snowflake collision], aion-cli `{ok:false,skipped:true}` no-op, never-throws on subprocess failure (5-min SIGKILL + 32KB cap copied from installer.ts). `api-key-writer.ts` now EXPORTS `WRITE_TARGETS`+`WriteTarget`; `index.ts` barrel re-exports the surface. Verify: cli-uninstall.test.ts **12/12**, full cli-installer suite **118/118** [no 267/268-01 regression], `tsc` 320 = baseline zero delta. **CODE ONLY — NOT DEPLOYED** (livinityd=tsx, no build; PURE MODULE — the `cliInstaller.uninstall` adminProcedure + `httpOnlyPaths` + `scheduleAgentRefresh` wiring is 268-03). SUMMARY: `.planning/phases/268-.../268-02-SUMMARY.md`.
- [x] 268-03-PLAN.md — router + transport: cliInstaller.sendAuthInput + cliInstaller.uninstall adminProcedures + httpOnlyPaths + index.ts wiring (uninstall fires agent-refresh) — ✅ **SHIPPED 2026-06-14** (3 commits, TDD test→feat + feat: `5f2bba7a` RED [router T7/T14 drift-locks → 9 routes + 11 new behavior tests T40-T45/T50-T55 + `common.test.ts` Tests 19/20; RED proven `No procedure found on path "sendAuthInput"/"uninstall"`], `8e0e3e39` GREEN Task 1, `57188438` Task 2). `cli-installer-router.ts` — `SendAuthInputInput` zod [name 1..64, **code 1..4096**]; `sendAuthInputFn`/`uninstallFn` DI seams + `PRECONDITION_FAILED` default + empty-injection stubs; 2 adminProcedures after `agentRefreshStatus`, **both `assertWhitelisted` FIRST (RCE boundary)**; **`sendAuthInput` returns `{ok}`, does NOT fire agent-refresh** [success arrives later on the child's own exit, not the stdin write]; **`uninstall` fires `triggerAgentRefresh()` ONLY on `result.ok`** → removed agent DISAPPEARS from `/api/agents` (E-6), a failed uninstall does NOT churn. `common.ts` — both routes → `httpOnlyPaths` [WS-survival after `systemctl restart livos`, B-12/X-04]. `index.ts` — imports `sendAuthInput`+`uninstallCli` from the barrel; wires `sendAuthInputFn` [stateless module-fn, mirrors `writeApiKeyFn`, no redis closure] + `uninstallFn` [`uninstallCli` resolves home/fs/PATH internally; reuses the wired `scheduleAgentRefreshFn`]; boot log lists both. Verify: router **48/48**, full cli-installer suite **130/130** [no 267/268-01/02 regression], `common.test.ts` tsx script **20/20**, `tsc` 320 = baseline zero delta [no error references the 3 edited files]. **CODE ONLY — NOT DEPLOYED** (livinityd=tsx, no build; transport only — 268-04 wires the dialog/panel to these routes). SUMMARY: `.planning/phases/268-.../268-03-SUMMARY.md`.
- [x] 268-04-PLAN.md — UI: dialog paste-back phase (URL + code field) + Uninstall button+confirm + bridge 'cli-uninstall' case + panel Remove button — ✅ **SHIPPED 2026-06-14** (2 commits, feat ×2: `1f175be2` Task 1 — `cli-auth-dialog.tsx` NEW `{kind:'auth-paste-back'}` Phase + `branchToAuthPhase` `case 'paste-back'`; widened `getDeviceCode` poll [`devicePolling`] so paste-back renders the device-style URL block; `sendAuthInputM`+`uninstallM` mutations; `pasteCode`/`pasteError`/`confirmUninstall` state; a `pasteStarted` ref fires `authM.mutateAsync({name})` ONCE — **its `{ok}` resolution [the login child's own exit after consuming the stdin code] is the FINAL completion signal → `{kind:'ready'}`, NOT the paste**; `handleSubmitPasteCode` → `sendAuthInputM.mutateAsync({name, code})` then **`setPasteCode('')` immediately on success OR failure** [bearer-secret clear-after-submit, E-9/T-268-16]; render = "1 · Open this link" `hostOf(url)` + explicit `Open link ↗` [NEVER auto-navigate, T-268-17] + "2 · Paste the code" masked `<PasswordInput>`; `Submit code` footer; **Uninstall affordance** only when `detectQ.data?.detected===true` & NOT install/auth/ready/loading → `text-destructive2-lightest` "Remove `<label>`" → inline two-step confirm [T-268-19] → `handleUninstall` → `uninstallM.mutateAsync({name})` → on ok `{kind:'ready', applied:false}` so the EXISTING 267-03 `agentRefreshStatus` "Applying…" poll runs while the agent disappears [E-6]; Retry resets `pasteStarted`/`confirmUninstall`/`pasteCode`. `acfe9c39` Task 2 — `use-cli-auth-bridge.ts` NEW `cli-uninstall` case → `openCliAuthDialog({cli, mode:'auth'})` [detail union unchanged; origin-check + `/^[a-z0-9-]+$/ && INSTALLABLE_CLIS` NAME-only RCE gate runs BEFORE the switch, T-268-18]; `local-agents-install-section.js` Remove button gated `!authHidden` [aion-cli never shows one], visibility toggle in `setRowState`, `hydrate` posts NAME-only `cli-uninstall`). **Deviation:** 1 auto-fix [Rule 1] — a JSX comment's literal `*/` closed the comment early → esbuild error; reworded, fixed before commit. Verify: `pnpm --filter @livos/config build` + `pnpm --filter ui build` (vite DEPLOY GATE) both exit 0; `node --check` panel JS OK; all acceptance greps hit; **267 device/apikey/browser branches + agentRefreshStatus poll preserved**. **CODE ONLY — NOT DEPLOYED** (deploy: config+ui vite build then release-based `bash /opt/livos/update.sh`; AionUi panel patch ships with aionui-patches apply; HARD-REFRESH/clear PWA SW). **Operator UAT (Mini PC):** paste-back (claude bare login → URL+code → ready ~10s no terminal) + uninstall (detected CLI Remove → confirm → "Applying…" → gone from `/api/agents`). **A5 deferred (RESEARCH OQ1):** bare-`claude` under-a-pipe vs TTY — node-pty fallback only if live UAT shows no URL. SUMMARY: `.planning/phases/268-.../268-04-SUMMARY.md`.

### Phase 269: CLI agent UX fixes — manual apply (no restart storm) + claude OAuth paste-back via node-pty + auth-gated agent list

**Goal:** Fix the four CLI-agent UX problems surfaced by Phase 267/268 LIVE UAT on the Mini PC (v44.3), per operator-locked decisions. **(WS1) MANUAL APPLY — kill the restart storm:** Phase 267-03's auto `sudo systemctl restart liv-assistant` fires on EVERY install/auth/uninstall; each restart takes AionUi (:3020) down ~40s → the whole `/liv/api/*` surface returns 502 (operator saw repeated "Bad gateway 502 / AionUi çöktü" doing a few uninstalls + a claude auth). REMOVE the automatic restart from the auth/setApiKey/uninstall success paths; instead track "pending agent changes" and expose a user-triggered `cliInstaller.applyAgentChanges` route + an "Apply changes (refresh Liv AI)" button in the dialog/panel, so the operator batches actions and triggers exactly ONE restart when ready. Keep the debounced restart helper (agent-refresh.ts) but call it ONLY from the explicit apply route. **(WS2) CLAUDE OAUTH PASTE-BACK via node-pty:** the bare `claude` login under a pipe goes into `--print` mode and errors (`"no stdin data received in 3s… Input must be provided … when using --print"`) — confirmed live; it needs a real TTY. Run the login under node-pty (already a livinityd dep, pty-sessions/session.ts) so it shows the interactive `Paste code here if prompted` prompt; surface the URL (267 stdout parse), write the operator-pasted code to the pty (`pty.write(code + '\r')`), detect completion. Keep the API-key fallback. The live-child registry (268-01) must handle a pty handle, and `sendAuthInput` must write to the pty when the login is pty-backed. **(WS3) AUTH-GATED AGENT LIST:** AionUi lists every INSTALLED CLI regardless of auth (operator: "CLI auth'lu değil ise neden gözüküyor main sayfada?"), so unauthed agents appear in the picker but fail in chat. Hide or clearly badge ("not signed in") the unauthed agents using the `liv:cli:auth:<name>` status, so only usable (authed) agents are presented as ready. **(WS4) USE AionUi's BUILT-IN ICONS:** the Local Agents panel ("aşağıda") still renders monogram fallbacks for CLIs with no 267-04 SVG, but AionUi already ships a COMPLETE logo set served at `/liv/api/assets/logos/{category}/{name}.{svg,png}` (observed: ai-major/claude.svg, tools/coding/cursor.png, brand/aion.svg, gemini.svg, opencode-light.svg, codex.svg). Point the agent-logo map (267-04 agent-logos.tsx + the panel renderIcon) at AionUi's authoritative asset URLs first, with the 267 SVG / monogram as final fallback.
...
### Phase 270: Unify the Local Agents panel into one list + fix Aion CLI still showing in the panel Detected section

**Goal:** Redesign the "Local Agents" panel (the AionUi-patch `scripts/aionui-patches/local-agents-install-section.js`, which renders inside the Liv AI `/liv` iframe) per operator UAT of v44.6. **(A) ONE UNIFIED LIST:** today the panel has TWO separate sections — a top "Detected" strip (the agents AionUi currently sees) and a separate "Available to Install" grid (the 20 SUPPORTED_CLIS with Install/Auth/Remove). The operator wants ONE place: every CLI shown together in a single list, where the ALREADY-installed/detected ones and the NOT-yet-installed ones live in the SAME place (installed ones show Auth/Remove, not-installed ones show Install) — no duplicate "Detected" strip above. Operator: "Burada gözüksün her şey, eğer indirilmeyenler vs varsa yine aynı yerde olsun." **(B) FIX AION STILL SHOWING:** Phase 269.1 hid Aion CLI from the WS3 `/liv/api/agents` overlay (the Liv AI chat PICKER), but the operator reports Aion CLI STILL appears in the Local Agents PANEL's "Detected" strip — because that panel renders its agent list from a DIFFERENT source than the overlay-filtered picker (investigate: does the panel fetch `/api/agents` without going through the `@liv_agents` Caddy carve-out / the livinityd overlay route, or read AionUi's list directly, or is it stale SW cache?). The unified list MUST also drop Aion CLI (binary_name 'aion') consistently with 269.1, OR route the panel's agent data through the same overlay so the hide is enforced in one place. Verify on the live box that Aion CLI is gone from BOTH the picker AND the panel after a SW clear + Re-detect.
...
- [x] 285-04-PLAN.md — Item 3: remove the Back-That-Mac-Up/Time Machine notice (backend + UI + integration test) ✅ 2026-06-18 (`353c3ada`) — see [285-04-SUMMARY.md](phases/285-umbrel-leftover-cleanup-and-docker-scroll-fix/285-04-SUMMARY.md)
