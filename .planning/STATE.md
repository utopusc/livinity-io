...
status: Phase 60 (Public Endpoint + Rate-Limit) Wave 2 (60-03) COMPLETE — Relay (Server5:4000) extended with `findAdminTunnel(registry, pool)` helper (queries `users.role='admin'` — defeats username-spoofing T-60-20) + `sendBrokerTunnelOffline(res)` Anthropic-spec 503 JSON envelope; `server.ts` dispatch inserted immediately after `parseSubdomain` to special-case `Host: api.livinity.io` → admin tunnel forward with `/u/<adminUserId>` URL rewrite + checkQuota path mirroring existing per-user proxy with Anthropic-spec 429 body; 11/11 vitest tests GREEN (8 unit + 3 dispatch integration via vi.mock of request-proxy/offline-page/custom-domains/bandwidth/health); deployed to Server5 via 1 scp + 1 batched ssh (admin-tunnel.ts new + server.ts modified into `/opt/platform/relay/src/`; pm2 restart relay clean uptime 0→3s; `/health` HTTP/1.1 200; `Host: api.livinity.io` probe HTTP/1.1 503 with body `{"type":"error","error":{"type":"api_error","message":"broker tunnel offline"}}` — NOT-502 ⇒ routing chain works; `livinity.io` HTTP/2 200 unchanged). vitest@^2.1.2 added as dev-dep (Rule 3 deviation: zero existing test runner in `platform/relay/`; runtime D-NO-NEW-DEPS preserved — vitest is dev-only and never deployed). FR-BROKER-B2-01 partially satisfied (relay routes correctly; Wave 3 Caddyfile + DNS completes end-to-end). Sacred SHA `4f868d318abff71f8c8bfbcf443b2393a553018b` byte-identical across plan (3 sample points). Wave 3 (60-04 — Caddyfile patch + DNS A record + broker IP-guard removal) unblocked.
last_updated: "2026-05-03T05:14:00Z"
last_activity: "2026-05-03 — Phase 60 Plan 03 (Wave 2 — Relay api.livinity.io routing) executed; task commits `591d30a2` RED + `0e4a55b0` GREEN admin-tunnel helpers; `8cdb3024` RED + `9cf46c25` GREEN server.ts dispatch; SUMMARY at `.planning/phases/60-public-endpoint-rate-limit/60-03-SUMMARY.md`"
progress:
  total_phases: 8
  completed_phases: 4
  total_plans: 44
...
Phase: 60 IN PROGRESS — Wave 0 (60-01 diagnostic), Wave 1 (60-02 binary swap), and Wave 2 (60-03 relay extension) COMPLETE. Awaiting Wave 3 (60-04 — Caddyfile patch + DNS A record + broker IP-guard removal).
Plan: 60-03 SHIPPED 2026-05-03 — Wave 2 relay extension: TDD RED→GREEN cycle for `findAdminTunnel(registry, pool)` (queries `SELECT id, username FROM users WHERE role = $1 LIMIT 1` with param `'admin'` — T7 SQL contract test asserts; defeats username-spoofing T-60-20) + `sendBrokerTunnelOffline(res)` (503 + `Content-Type: application/json` + `{type:'error', error:{type:'api_error', message:'broker tunnel offline'}}`); then second TDD cycle for `server.ts` dispatch wired immediately after `parseSubdomain(req.headers.host)` and before the `if (!username && req.url === '/health')` line — order: pool absent → 503; `findAdminTunnel` null OR ws.readyState !== 1 → 503; `checkQuota(redis, adminTunnel.userId)` not allowed → 429 Anthropic rate_limit_error JSON + Retry-After 86400; URL rewrite `req.url = '/u/' + adminTunnel.userId + req.url` (unless already /u/ prefixed); `proxyHttpRequest(adminTunnel, req, res, null)`. 11/11 vitest tests GREEN (T1-T8 unit + T9-T11 dispatch integration via vi.mock of request-proxy.js, offline-page.js, custom-domains.js, bandwidth.js, health.js); T11 regression test asserts `alice.livinity.io` UNCHANGED (registry.get('alice') called, URL not rewritten, pool.query NOT invoked). Build (`tsc`) + typecheck (`tsc --noEmit`) PASS. Deploy: 1 ssh probe (pre-deploy state capture: relay deploy dir layout, src/ dir layout, admin-tunnel.ts presence-check confirmed absent, /health 200 baseline, pm2 list, ecosystem.config.cjs script confirmed `src/index.ts` via tsx — no compile-on-server step needed) + 1 scp upload (`platform/relay/src/admin-tunnel.ts` new + `platform/relay/src/server.ts` modified → `/opt/platform/relay/src/`; remote md5 verified) + 1 batched ssh (pm2 restart relay → uptime 0s → 3s, status `online`, restart count 8→9; pm2 logs clean: `Schema applied`, `Custom domain cache warmed`, `Relay server listening on port 4000`, no errors; `/health` HTTP/1.1 200 + JSON body; api.livinity.io probe with `Host:` header injection → HTTP/1.1 503 + `application/json` + `{"type":"error","error":{"type":"api_error","message":"broker tunnel offline"}}` in 16ms — NOT-502 ⇒ dispatch fired correctly + findAdminTunnel ran cleanly + 503 envelope returned per plan; livinity.io HTTP/2 200 regression PASS). 3 Server5 ops total (1 probe + 1 scp + 1 deploy ssh) — fail2ban-conservative. Three deviations all auto-fixed: Rule 3 (no test runner: vitest@^2.1.2 added as dev-dep — runtime D-NO-NEW-DEPS preserved); Rule 3 (rsync absent on Windows git-bash: switched to scp — same functional outcome); Rule 2 (pre-deploy state probe added beyond plan as defensive ssh-probe gate). Task commits: `591d30a2` test RED + `0e4a55b0` feat GREEN (admin-tunnel.ts/test); `8cdb3024` test RED + `9cf46c25` feat GREEN (server.ts dispatch).
Status: Phase 60 Wave 2 COMPLETE. Sacred SHA `4f868d318abff71f8c8bfbcf443b2393a553018b` byte-identical at plan start, mid-plan (after Task 1 GREEN), and plan end (after Server5 deploy + smoke). FR-BROKER-B2-01 partially satisfied (relay routes api.livinity.io → admin tunnel; offline-tunnel returns Anthropic-spec 503 JSON; Wave 3 completes the requirement end-to-end with Caddyfile + DNS). Wave 3 (60-04) unblocked. SUMMARY at `.planning/phases/60-public-endpoint-rate-limit/60-03-SUMMARY.md`.
Last activity: 2026-05-03 — Phase 60 Plan 03 (Wave 2 — Relay api.livinity.io routing) executed; task commits `591d30a2` + `0e4a55b0` + `8cdb3024` + `9cf46c25`; SUMMARY at `.planning/phases/60-public-endpoint-rate-limit/60-03-SUMMARY.md`
