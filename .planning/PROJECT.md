...
**Target features (9 phases planned, P209-P217):**
- **P209 — openclaw → Claude CLI reuse + Haiku 4.5 default** (TODAY, 5 min ops): liv-claw-gateway switches default model from `openrouter/nvidia/nemotron-nano-9b-v2:free` to `anthropic/claude-haiku-4-5` via local `claude-cli` backend reusing `/root/.claude/.credentials.json` (subscription path, zero code, zero new auth).
- **P210 — Subdomain canonical format + 3 critical relay/install bugs**: (210.1) relay `parseSubdomain` misroutes hyphen-format → falls through to offline page rendering wrong tenant; (210.2) `provisionAppSubdomain` silent-null → Caddy hostname mismatches DNS; (210.3) `REDIS_PLATFORM_URL` undeclared → silent ReferenceError drops every install_history event.
- **P211 — MCP/App install reliability + auto-install MCP**: (211.1) dual-writer collision at `liv:mcp:config` (JSON-string vs HASH); (211.2) `EnvironmentOverridesDialog` replaces opaque `dependency_missing` toast; (211.3) admin-only install gate (`is_admin` flag enforced on `apps.install` + `installV37` + `/api/admin/apps`).
- **P212 — Admin panel auth + data model**: Supabase migration adds `is_admin BOOLEAN` to `public.users`, seeds `hello@bruceoz.com`, adds RLS so non-admins only see their own row. 6 read-only admin API routes under `platform/web/app/api/admin/` (summary, users, tunnels, apps, bandwidth, install-failures). Heartbeat persistence audit.
- **P213 — Admin panel UI**: 6 admin pages (`/admin`, `/admin/users`, `/admin/users/[id]`, `/admin/tunnels`, `/admin/apps`, `/admin/store`, `/admin/walkthrough`) on shadcn/ui + recharts + Supabase server client. Static-HTML rewrites kept (operator decision 2026-05-26 — `/dashboard.html`, `/auth.html`, `/login.html` untouched; only `/admin/**` new).
- **P214 — Store redesign: admin-only gate + UX polish**: `/store/**` middleware redirects non-admin → `/dashboard`. Catalog sync from `utopusc/livinity-apps` repo into Supabase `public.apps` (304 candidate manifests). Featured/verified curation UI. Detail page redesign.
- **P215 — One-click MCP/app install in store + walkthrough docs**: Store "Install" button → Mini PC bridge → Phase 211 install path with SSE progress UI. `/admin/walkthrough` docs (3 guides: Docker app, MCP, custom non-Docker app).
- **P216 — Cloudflare audit + automation**: Enumerate current DNS records via CF API → `.planning/cloudflare-state.md`. **Live audit** of per-user wildcard cert (`*.livinity.io` via Server5 Caddy on-demand TLS + relay `/internal/ask`) — operator answer "Bilmiyorum / araştır" defers this confirmation to this phase. Optional Terraform/wrangler config.
- **P217 — E2E UAT + verification**: Operator walks every Phase 209-216 deliverable; UAT-CHECKLIST.md sections per phase; fix any FAIL, archive milestone.

**Estimated effort:** 18-22 days wall-clock (~120-150 hours solo).

**Locked decisions for v41 entry:**
- **D-V41-ADMIN-SEED** — `hello@bruceoz.com` is the single admin seed (operator answer 2026-05-26).
- **D-V41-STATIC-HTML-KEEP** — Existing `/dashboard.html`, `/auth.html`, `/login.html` static rewrites in `next.config.ts` Phase 146 are NOT ported to Next.js. Only `/admin/**` is a new Next.js route group (operator answer 2026-05-26).
- **D-V41-BANDWIDTH-ROLLUPS** — Time-series stored in plain Supabase rollup tables (`hourly_bandwidth`, `daily_bandwidth`) — no TimescaleDB extension, no external Grafana/Influx (operator answer 2026-05-26 — Recommended path).
- **D-V41-RELAY-STATE-UNKNOWN** — Server5 relay status not confirmed at milestone open; Phase 210 entry must include a single batched SSH probe to confirm relay path before patching subdomain bugs (operator answer 2026-05-26).
- **D-V41-CF-CERT-AUDIT-DEFERRED** — Per-user CF wildcard cert health is audited *live* in Phase 216 (not assumed working). If broken post-Vercel-migration, fix lands in P216 same phase.
- **D-V41-CLAUDE-CLI-REUSE** — openclaw Liv AI chat uses local `claude-cli` backend (Anthropic-sanctioned reuse path per 2026-05-26 operator confirmation). No broker, no API key, no BYOK — sacred subscription rule holds.
- **D-V41-ADMIN-ONLY-STORE** — `livinity.io/store` becomes admin-only. Public marketing landing replaces the old open catalog. Reverts the v37 "public store" stance.
- **D-V41-NO-SERVER5-ADMIN-MIGRATION** — Admin panel lives ONLY on Vercel from day one. Server5 web app may keep running through a transition but receives no new admin code.
- **D-V41-NO-SUPABASE-AUTH-REPLACEMENT** — Use what's already wired (`auth` schema is fully present in Supabase project `qlsalsyqjichtpjitldi`).

**Phase plan source:** `.planning/v41-DRAFT.md` (operator-authored 2026-05-26, ~260 lines, includes 3 research artifacts at `.planning/research/2026-05-26-store-admin/` and a 4th at `.planning/research/2026-05-26-computer-use-perf/R5-broker-tool-routing.md`). REQUIREMENTS.md and ROADMAP.md derived from this draft 2026-05-26.

**Next action:** `/gsd-discuss-phase 209` (Phase 209 = 5-min Claude CLI reuse — ship TODAY) OR `/gsd-plan-phase 209` to skip discussion.

---

## Previous Milestone: v31.0 Liv Agent Reborn (OPENED 2026-05-04, superseded 2026-05-26 by v41)
...
---
*Last updated: 2026-06-17 — v46.0 "Trust, Safety, Compliance & Cost Controls" opened (Phases 279-284; runs alongside in-flight v45). Plan in `.planning/milestones/v46/` (PROJECT/REQUIREMENTS/ROADMAP). Driven by the session Cloudflare & Cost Risk Report + an operator legal-documents request; US/Delaware governing law, English, `[PLACEHOLDER]` entity fields. Phase 279 (Terms/Privacy/AUP/Cookies/Refund + `/legal` pages + signup & footer wiring) executed in-session. Prior footer retained below.*
