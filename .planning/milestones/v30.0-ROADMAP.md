...
- [x] **Phase 60: B2 Public Endpoint (`api.livinity.io`) + Rate-Limit Perimeter** — SHIPPED 2026-05-03. 5/5 plans complete (60-01..05). 2/2 requirements satisfied (FR-BROKER-B2-01 + FR-BROKER-B2-02). Sacred file SHA `4f868d318abff71f8c8bfbcf443b2393a553018b` byte-identical at 16 sample points across phase. Server5 Caddy custom build (caddy-ratelimit + caddy-dns/cloudflare) + api.livinity.io block (rate_limit zones bearer 60/min + ip 30/min + flush_interval -1 + Anthropic-spec 4-field 429 + log) + DNS A record + relay api.livinity.io dispatch (admin tunnel + sendBrokerTunnelOffline 503) + broker IP-guard removed at source (Mini PC deploy deferred to Phase 63 update.sh per plan note). Smoke battery: 70/100 × 429 with full 4-field Anthropic body + Retry-After:59 (success criterion 3 PASS); LE E8 cert openssl Verify=0 + DNS resolves correctly from 2 resolvers (success criterion 4 PASS); Bearer-authed live test deferred to Phase 63 (success criteria 1 + 2 — chain verified live up to broker via 30 × 503 sendBrokerTunnelOffline envelopes during blast). PHASE-SUMMARY at `.planning/phases/60-public-endpoint-rate-limit/PHASE-SUMMARY.md`.
- [x] **Phase 61: C3+D1+D2 Rate-Limit Headers + Model Aliases + Provider Interface Stub** — SHIPPED 2026-05-03. 4/4 plans complete (61-01..04). 7/7 requirements satisfied (FR-BROKER-C3-01..03 + FR-BROKER-D1-01..02 + FR-BROKER-D2-01..02). Sacred file SHA `4f868d318abff71f8c8bfbcf443b2393a553018b` byte-identical at 12 sample points across phase. New `rate-limit-headers.ts` (123 LOC) — prefix-loop forward of all anthropic-* + retry-after on Anthropic route; 6-canonical translation to x-ratelimit-* on OpenAI route with RFC-3339-to-duration-string reset format. Both passthrough streaming branches REORDERED (provider.streamRequest before flushHeaders) for R9 mitigation. New `alias-resolver.ts` (Redis-backed, 5s TTL cache, hardcoded fallback) + `seed-default-aliases.ts` (10 aliases via SETNX) — wired into BOTH broker routes (Anthropic route bug fix: `body.model='opus'` no longer 404s). New `BrokerProvider` interface + `AnthropicProvider` concrete + 3 stubs (OpenAI/Gemini/Mistral throw NotImplementedError) + grep-guard test. 53 new tests across phase; broker suite 94→147 GREEN; 5-run determinism gate GREEN at phase end. D-NO-NEW-DEPS preserved across all 4 plans. PHASE-SUMMARY at `.planning/phases/61-spec-compliance-aliases-provider/PHASE-SUMMARY.md`.
- [x] **Phase 62: E1+E2 Usage Tracking Accuracy + Settings UI** — SHIPPED 2026-05-03. 5/5 plans complete (62-01..05). 5/5 requirements satisfied (FR-BROKER-E1-01..03 + FR-BROKER-E2-01..02). Sacred file SHA `4f868d318abff71f8c8bfbcf443b2393a553018b` byte-identical at every plan boundary + end of phase. New `broker_usage.api_key_id` nullable UUID FK column with partial index + 8-param insertUsage; capture middleware reads `req.apiKeyId` from Phase 59 bearer middleware; tRPC `usage.getMine`/`getAll` accept apiKeyId/api_key_id filter; UI ships Stripe-style API Keys CRUD section (`<ApiKeysSection>` + Create/Revoke modals) + "Filter by API key" Select dropdown in `<UsageSection>` and 4th admin filter chip in `<AdminCrossUserView>` with localStorage persistence (`livinity:usage:filter:apiKeyId`); FR-BROKER-E1-03 OpenAI streaming integration test GREEN. 76 new tests across phase (30 backend + 46 UI); backend usage-tracking 45/45 + UI settings/_components 52/52. D-NO-NEW-DEPS preserved (zero `package.json` / lockfile changes). PHASE-SUMMARY at `.planning/phases/62-usage-tracking-settings-ui/PHASE-SUMMARY.md`.
...
- [x] 60-04-PLAN.md — Wave 3 Caddyfile addition + DNS A record + broker IP-guard removal — SHIPPED 2026-05-03 (commits `f57e5269` chore drift-reconcile + `965d6011` feat api.livinity.io block w/ rate_limit + flush_interval -1 + Anthropic-spec 429 + log; `56b4dd7c` docs DNS-INSTRUCTIONS.md; `a240f81f` feat IP-guard removal source-only — Mini PC deploy deferred to Phase 63 update.sh; deployed to Server5 with pre-swap backup `/etc/caddy/Caddyfile.bak.20260503-072328`; caddy validate PASS exit 0; reload SUCCEEDED on retry after Rule 1 perm fix on /var/log/caddy/api.livinity.io.log; smoke regression livinity.io/apps/changelog/bruce all HTTP/2 200; api.livinity.io TLS 1.3 PASS via curl --resolve + HTTP/2 stream OPENED for /v1/messages; DNS A record verified ALREADY EXISTS via dig 1.1.1.1+8.8.8.8; LE cert pre-issued E8 expires 2026-06-17; T-60-34 grep "authorization" log = 0; typecheck delta = 0; vitest broker 94/94 GREEN; sacred SHA byte-identical 3 sample points; FR-BROKER-B2-01 + FR-BROKER-B2-02 satisfied)
- [x] 60-05-PLAN.md — Wave 4 phase-60-smoke.sh + 60-SMOKE-RESULTS.md + sacred file final gate — SHIPPED 2026-05-03 (commits `d01f792a` smoke script + `2002853f` SMOKE-RESULTS.md; smoke from Server5 per checkpoint SOURCE_IP=server5 to avoid dev-box self-lockout per RESEARCH.md Pitfall 8; §1+§2 PASS DNS+TLS; §3 SKIP per checkpoint TOKEN_STATE=skip + MINI_PC_DEPLOYED=no — chain verified up to broker via 30 × 503; §4 PASS 70/100 × 429 + 4-field Anthropic body + Retry-After:59 — perimeter empirically proven; §5 livinity.io/apps/changelog/bruce all 200 — relay.livinity.io 503 pre-existing per Wave 0 §Notes 4 NOT a regression; §6 T-60-34 re-investigated INTACT — Caddy auto-redacts Authorization VALUE to "REDACTED" zero Bearer/liv_sk leaks; §7 sacred-SHA dev-box re-run authoritative `4f868d318abff71f8c8bfbcf443b2393a553018b` — Phase 60 milestone gate PASS; SUMMARY at `.planning/phases/60-public-endpoint-rate-limit/60-05-SUMMARY.md`; PHASE-SUMMARY at `.planning/phases/60-public-endpoint-rate-limit/PHASE-SUMMARY.md`)
...
Plans:
- [x] 62-01-PLAN.md — Wave 1 Backend test scaffolds + schema migration + database.ts 8-param refactor (FR-BROKER-E1-01) — SHIPPED 2026-05-03 (commits 434967c0 + fd0a75a6); 5 schema-migration tests GREEN + 3 RED handed off
- [x] 62-02-PLAN.md — Wave 2 Capture middleware reads req.apiKeyId at response time (FR-BROKER-E1-02) — SHIPPED 2026-05-03 (commit 54e7289d); 8/8 capture-middleware GREEN incl. 2 new + bonus E1-03 GREEN
- [x] 62-03-PLAN.md — Wave 2 tRPC routes accept apiKeyId filter input (FR-BROKER-E2-02 backend) — SHIPPED 2026-05-03 (commits 39527673 + 8d151c84); 7/7 routes.test.ts GREEN incl. 2 new
- [x] 62-04-PLAN.md — Wave 2 ApiKeysSection + Create/Revoke modals + ai-config.tsx FLAT insertion (FR-BROKER-E2-01) — SHIPPED 2026-05-03 (commits ec44608c + fd7ba777); 23/23 GREEN
- [x] 62-05-PLAN.md — Wave 3 Usage filter dropdown + admin filter + FR-BROKER-E1-03 integration test + final phase gate — SHIPPED 2026-05-03 (commits 9ecc7545 + f1cfa8f4); 10 use-usage-filter + 10 usage-section invariants GREEN + FR-BROKER-E1-03 verified GREEN
