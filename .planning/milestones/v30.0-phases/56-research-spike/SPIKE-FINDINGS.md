...
**Plans contributing:** 56-01 (Q1+Q2+Q7), 56-02 (Q3+Q4+Q5+Q6), 56-03 (cross-cuts), 56-04 (synthesis — this pass)
**Sacred file:** `nexus/packages/core/src/sdk-agent-runner.ts`
**Sacred file SHA at end of spike:** `4f868d318abff71f8c8bfbcf443b2393a553018b` (expected `4f868d318abff71f8c8bfbcf443b2393a553018b`) — PASS
...
Single-table snapshot of all 7 question verdicts plus 3 cross-cut verdicts. Phase 57+ planners read this section first and dive into the relevant Q-block for full rationale.

| Question | Verdict (one-sentence) | Integration Point | Phase Impact |
|----------|------------------------|-------------------|--------------|
| Q1: Passthrough strategy | **HTTP-proxy direct** to `api.anthropic.com/v1/messages` via Node 22 builtin `fetch()` with per-user OAuth subscription `access_token` extracted server-side (zero new npm deps) | `livinity-broker/router.ts:36-187` (mode dispatch + new `passthrough-anthropic.ts`) | Phase 57 |
| Q2: Tools forwarding | **Forward client `tools[]` verbatim** in passthrough; OpenAI route translates shape; agent mode keeps existing ignore-warn | `router.ts:66-70` (delete in passthrough) + `openai-router.ts:110-124` (translator) | Phase 57 |
| Q3: Agent-mode opt-in | **Both** URL-path `/u/<id>/agent/v1/...` AND header `X-Livinity-Mode: agent`; path takes precedence; default = passthrough | `router.ts:36` (dual route registration) + `openai-router.ts:71-283` | Phase 57 |
| Q4: Public endpoint | **Server5 Caddy** + new `api.livinity.io` block + `caddy-ratelimit` plugin (custom `xcaddy` build) + LE on-demand TLS | `platform/relay/Caddyfile` (top of file) | Phase 60 |
| Q5: Key rotation | **Manual revoke + recreate**, **opt-in keys** (no auto-key on signup); `revoked_at`-driven kill-switch matches FR-BROKER-B1-01 schema 1:1 | livinityd new tRPC routes (`createKey`/`listKeys`/`revokeKey`) + `api_keys` PG table | Phase 59, 62 |
| Q6: Rate-limit policy | **Edge handles abuse, broker handles transparency** — Caddy `caddy-ratelimit` perimeter from Q4; broker forwards Anthropic upstream rate-limit headers verbatim; broker emits ZERO own 429s in v30 | `router.ts:158-185` (existing 429 forward) + Caddyfile `rate_limit` zone | Phase 60, 61 |
| Q7: Agent-mode streaming | **Sacred file UNTOUCHED in v30** — passthrough (Q1) bypasses sacred file entirely for external streaming; agent mode keeps current aggregation (acceptable per Phase 51 deploy-layer fix); D-30-XX safety net deferred to v30.1+ if pain re-surfaces | `sdk-agent-runner.ts:378-389` (read-only ref); passthrough delivered via `router.ts:88` SSE branch | Phase 57, 58 |

**Cross-Cut Verdicts:**

- **D-NO-NEW-DEPS:** **YELLOW** — Zero new npm packages required by any Q1-Q7 primary path (`package.json` budget preserved); two non-npm Caddy/Go infra deps flagged for Phase 60 explicit budget (`caddy-ratelimit` plugin + `xcaddy` build tool).
- **Sacred File SHA Stability:** **PASS** — `4f868d318abff71f8c8bfbcf443b2393a553018b` byte-identical across all three executed plans (56-01, 56-02, 56-03) and confirmed at end of 56-04.
- **D-51-03 Re-Evaluation:** **Not needed in v30** — Q1 passthrough structurally bypasses sacred file for external clients (the original identity-contamination context); Q7 confirms agent mode acceptable per Phase 51 deploy-layer fix; v30.1+ D-30-XX safety net retained if internal-chat pain re-surfaces post-v30.
...
In one phrase: **edge handles abuse, broker handles transparency.** The broker emits ZERO of its own 429s in v30 — every 429 the client sees originates from EITHER the edge perimeter (Caddy) OR upstream Anthropic. **This resolves the open item in FR-BROKER-B2-02 (which previously deferred broker-side bucket pending this spike)** as follows: broker-side token bucket = NO; rate-limit perimeter = YES (edge per Q4); fine-grained = transparent forward from upstream Anthropic.
...
3. **Resolves FR-BROKER-B2-02's open item the simplest way.** The requirement said default = the user's Anthropic subscription rate forwarded transparently, with broker-side token-bucket left open pending Phase 56 spike. The verdict: the user's Anthropic rate IS the truth, broker is transparent, broker-side bucket is NOT in v30. Future v31+ can add an opt-in broker bucket if multi-tenant fairness becomes a real requirement; v30 schema (FR-BROKER-B1-01) is forward-compatible with future additions like `api_keys.bucket_capacity`.
...
**Verdict: PASS.** Sacred file `nexus/packages/core/src/sdk-agent-runner.ts` is byte-identical at SHA `4f868d318abff71f8c8bfbcf443b2393a553018b` after plans 56-01, 56-02, 56-03, AND 56-04 ran. No drift introduced by spike. Sacred boundary preserved.

| Field         | Value                                          |
| ------------- | ---------------------------------------------- |
| Expected SHA  | `4f868d318abff71f8c8bfbcf443b2393a553018b`     |
| Observed SHA (end of 56-04) | `4f868d318abff71f8c8bfbcf443b2393a553018b` |
| Match?        | YES — byte-identical                           |
| `git status`  | `nothing to commit, working tree clean` (re: sacred file) |
...
**Per-plan SHA stability log:**

| Plan | After task | SHA | Match? |
|------|-----------|-----|--------|
| 56-01 | Task 1 (Q1) | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |
| 56-01 | Task 2 (Q2) | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |
| 56-01 | Task 3 (Q7) | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |
| 56-02 | Task 1 (Q3 + Q5) | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |
| 56-02 | Task 2 (Q4)      | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |
| 56-02 | Task 3 (Q6)      | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |
| 56-03 | Task 1 (D-NO-NEW-DEPS Audit)        | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |
| 56-03 | Task 2 (Sacred SHA Stability check) | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |
| 56-03 | Task 3 (D-51-03 Re-Evaluation)      | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |
| 56-04 | Task 1 (Synthesis)                  | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |
| 56-04 | Task 2 (Validation + final SHA)     | `4f868d318abff71f8c8bfbcf443b2393a553018b` | ✓ |

Sacred file was NOT edited / written / staged in any Plan 56-XX task. Sacred boundary preserved across the entire spike.
...
**Action item routing:** Verdict (a) "Not needed in v30" → no further v30 action item. The Q7 D-30-XX candidate row remains the v30.1+ safety net. Phase 56-04 SUMMARY's "Deferred items" section lists D-30-XX explicitly so it isn't forgotten. Phase 63 UAT script (FR-VERIFY-V30-* coverage) implicitly covers internal-chat self-identification because internal AI Chat is part of the LivOS feature set Phase 63 verifies — if a regression surfaces there, it surfaces D-30-XX naturally.

---

## Decisions Log (D-30-XX entries — copy into STATE.md Locked Decisions)

These are the v30.0-locked decisions emitted by the Phase 56 spike. Each entry is self-contained: a Phase 57+ planner reading STATE.md must understand the decision without opening this document. Format: one-line decision + 2-3 sentence rationale + source pointer.
...
Per `56-RESEARCH.md` Validation Architecture, each Q-verdict block must contain three elements: (a) file:line code reference; (b) external source URL cited; (c) ≥2 alternatives considered. This table cross-references each Q to its evidence and marks pass/fail per cell.

| Q | (a) file:line cited | (b) external URL cited | (c) alternatives count | Pass? |
|---|---------------------|------------------------|------------------------|-------|
| Q1 | `router.ts:36-187`, `passthrough-anthropic.ts` (new), `agent-runner-factory.ts:64-173`, `claude.ts` (Phase 39 ToS-fingerprint reference), `nexus/packages/core/package.json:33-34` | `docs.anthropic.com/en/api/messages` (Anthropic Messages API), Anthropic OAuth/credentials.json schema (refresh_token presence) | 3 (Strategy B SDK-direct REJECTED; Strategy C hybrid folded; pure HTTP-proxy w/ client-supplied token REJECTED via D-NO-BYOK) | ✓ |
| Q2 | `router.ts:66-70` (D-41-14 ignore-warn), `openai-router.ts:110-124` (D-42-12 ignore-warn), `router.ts:158-185` (Phase 45 forward) | `docs.anthropic.com/en/docs/agents-and-tools/tool-use/overview`, `docs.anthropic.com/en/docs/agents-and-tools/tool-use/computer-use`, `docs.anthropic.com/en/api/managing-api-keys` | 3 (status-quo ignore-warn REJECTED; selective-allowlist REJECTED; forward-in-both-modes REJECTED) | ✓ |
| Q3 | `router.ts:36`, `agent-runner-factory.ts:82` (existing X-LivOS-* namespace), `agent-runner-factory.ts:64-173`, `openai-router.ts:71-283` | `docs.anthropic.com/en/api/versioning` (anthropic-version/anthropic-beta header idiom), `docs.continue.dev/reference` (Continue.dev requestOptions.headers), `docs.cline.bot/provider-config/anthropic`, Bolt.diy `app/lib/modules/llm/providers/anthropic.ts`, Open WebUI `backend/open_webui/routers/openai.py` (ENABLE_FORWARD_USER_INFO_HEADERS) | 3 (Header-only REJECTED; Path-only viable but limited; no-opt-in REJECTED) | ✓ |
| Q4 | `platform/relay/Caddyfile` (Server5), Caddyfile insertion site (api.livinity.io block) | `developers.cloudflare.com/workers/configuration/routing/routes/` (Worker route requires proxied DNS), `caddyserver.com/docs/automatic-https` (on-demand TLS), `caddyserver.com/docs/modules/http.handlers.rate_limit` (custom build required), `raw.githubusercontent.com/mholt/caddy-ratelimit/master/README.md` (xcaddy install method), `developers.cloudflare.com/workers/platform/pricing/` (10ms CPU cap) | 3 (Cloudflare Worker+Durable Objects REJECTED; Caddy native-only REJECTED; Caddy+nginx-equivalent REJECTED as fictional) | ✓ |
| Q5 | FR-BROKER-B1-01 schema (`revoked_at`, `created_at`, `last_used_at`, `key_hash`, `key_prefix`), tRPC routes (createKey/listKeys/revokeKey — Phase 59 owns), `node:crypto` (randomBytes/sha256/timingSafeEqual), `pg@^8.20.0` | `docs.stripe.com/keys` (manual rotate), `openai-python` README (manual create+delete), `docs.anthropic.com/en/api/managing-api-keys` (manual disable/delete), OWASP API2:2023 Broken Authentication | 4 (Auto-90day-rotation REJECTED; short-TTL+refresh REJECTED; default-keyed REJECTED; toggle-gated auto-key folded into opt-in) | ✓ |
| Q6 | `router.ts:158-185` (Phase 45 forward pattern), `agent-runner-factory.ts:99-106` (agent-mode SSE adapter), Caddyfile `rate_limit` directive (per Q4) | `docs.anthropic.com/en/api/rate-limits` (12 Anthropic ratelimit headers including input/output token splits), RFC 7231 §7.1.3 / RFC 9110 §10.2.3 (Retry-After format), FR-BROKER-C3-02 (6 OpenAI ratelimit headers) | 3 (Forward-verbatim-only REJECTED no perimeter; broker-side Redis bucket REJECTED for v30; hybrid REJECTED) | ✓ |
| Q7 | `sdk-agent-runner.ts:264-270` (identity prepend READ-ONLY), `sdk-agent-runner.ts:340-355` (query call READ-ONLY), `sdk-agent-runner.ts:378-389` (aggregation loop READ-ONLY), `sdk-agent-runner.ts:440` (stream_event ignore READ-ONLY), `router.ts:88` (passthrough bypass site), `nexus/packages/core/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts:1003-1006` + `:2144-2150` (includePartialMessages option, SDKPartialAssistantMessage type) | Anthropic Messages SSE event spec (message_start/content_block_*/message_delta/message_stop), `@anthropic-ai/claude-agent-sdk@^0.2.84` published types | 3 (Plan candidate-A "fundamentally aggregates" framing REJECTED but outcome adopted; candidate-C "incidental aggregation" REJECTED; in-v30 sacred-file-edit REJECTED) | ✓ |

### Cross-Cut Validation

- **Sacred SHA stability:** Verified at start of each plan (notes from `56-RESEARCH.md`), at boundary of each plan task (per-plan SHA log table in Cross-Cuts §Sacred File SHA Stability), AND at end of phase (this synthesis pass). Final SHA: `4f868d318abff71f8c8bfbcf443b2393a553018b`. Expected: `4f868d318abff71f8c8bfbcf443b2393a553018b`. **PASS.**
- **D-NO-NEW-DEPS audit:** Package-by-package walked in 56-03 Task 1 (table in Cross-Cuts §D-NO-NEW-DEPS Audit covers 12 dependencies/runtimes); evidence in `notes-cross-cuts.md` plus inline grep output of `nexus/packages/core/package.json` lines 33/34/43/46 and `livos/packages/livinityd/package.json` lines 34/90/98/114. **PASS (npm side GREEN; non-npm Caddy/xcaddy delta YELLOW for Phase 60).**
- **Zero placeholder rows:** Confirmed — every Q-verdict block contains a concrete chosen path (no remaining unresolved tokens of the historical `T_B_D` form across the 7 question verdicts and Cross-Cuts). **PASS.**
- **All five sections present in canonical order:** Executive Summary → Q1 → Q2 → Q3 → Q4 → Q5 → Q6 → Q7 → Cross-Cuts → Decisions Log → Validation. **PASS.**

---

*Phase 56 spike COMPLETE. Synthesis (56-04) closes the spike with Executive Summary + Decisions Log (D-30-01..D-30-09) + Validation table. Ready for Phase 57+ implementation planning.*
