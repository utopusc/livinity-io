...
4. **Create livos/packages/ui/src/routes/ai-chat/voice-button.tsx:**
   ...
   **WebSocket Connection:**
   - On component mount (if voice is enabled), establish WebSocket connection:
     ...
     ```
     JWT is retrieved from `localStorage.getItem('jwt')` — the same pattern used by the terminal WebSocket in `livos/packages/ui/src/routes/settings/terminal/_shared.tsx:50`.
   - Handle messages:
