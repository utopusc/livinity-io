...
1. `rounded-6` (line 145, AppIcon) -> `rounded-radius-sm`
2. `text-13` (line 149, DialogDescription) -> `text-body-sm`
3. `opacity-50` (line 149, DialogDescription) -> remove `opacity-50`, add `text-text-secondary`
4. `border-white/10` (line 162, Public Access border-t) -> `border-border-default`
- Should follow Phase 3 dialog patterns (bg-dialog-content, border-border-subtle)
- Keep functional brand/status colors
- Do NOT modify: dependency selection logic, mutation handlers, dialog open/close

**default-credentials-dialog.tsx (~150 lines):**
1. `text-13` (line 139, checkbox label) -> `text-body-sm`
2. `text-white/40` (line 144, dont-show-again-notice) -> `text-text-tertiary`
3. `textClass` const (line 150): `text-13` -> `text-body-sm`, `text-white/40` -> `text-text-tertiary`
- Should follow Phase 3 dialog patterns
- Do NOT modify: credential display logic, checkbox state, launch handlers
...
Search for `opacity-50` in app-settings-dialog.tsx — should not exist (replaced with text-text-secondary).
Search for `text-white/40` in default-credentials-dialog.tsx — should not exist (replaced with text-text-tertiary).
Search for `rounded-6` in app-settings-dialog.tsx — should not exist (replaced with rounded-radius-sm).
  </verify>
  <done>
