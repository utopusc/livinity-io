...
| 242 | **Luse skill set for Claude Code** | `.claude/skills/luse/` with SKILL.md + sub-skills (click/type/screenshot/key/scroll). Operator uses via Claude Code agent inside Liv AI. Docs-only. | LOW | 4h | — (no Mini PC deploy) |
...
- **Wave D (sequential after 241):** 240 (Local Agents UI) — depends on the MCP registrar being in place
