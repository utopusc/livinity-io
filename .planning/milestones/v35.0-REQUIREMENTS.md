# v35.0 Requirements Archive (Design System Unification — UI/UX)

**Archived:** 2026-05-15 at v35.0 milestone close.

**Note on this archive:** v35.0 was opened as a design-system milestone with its acceptance criteria embedded directly in the master plan (`v35-DESIGN-SYSTEM-MILESTONE.md` § Acceptance criteria) rather than as discrete REQ-IDs in REQUIREMENTS.md. The 8 acceptance criteria (AC#1-8) functioned as v35.0's de-facto requirements and are tracked in `v35.0-MILESTONE-AUDIT.md`:

- AC#1 single design token source — PASS
- AC#2 single component library (ui-kit default) — PASS (22 v0.2.0 candidates routed to Phase 122+)
- AC#3 cross-surface visual parity — PASS at 92.5% match average
- AC#4 Geist + Geist Mono + Instrument Serif everywhere — PASS
- AC#5 light/dark/iridescent everywhere — PARTIAL (dark + iridescent stubs → v35.0-design-tokens-1.0.1 patch)
- AC#6 visual regression CI (Playwright + GH Actions) — PASS
- AC#7 inventory accuracy (post-migration tags) — PASS
- AC#8 operator-walked end-to-end UAT — PENDING OPERATOR (per-plan UAT blocks ship; operator paces walks)

The legacy REQUIREMENTS.md content below is **v31.0-era carry-state** — preserved verbatim from the live file at v35.0 close for historical continuity. For v36+, a fresh REQUIREMENTS.md will be created via `/gsd-new-milestone`.

---

# v31.0 Requirements — "Liv Agent Reborn" (legacy carry-state — preserved at v35.0 close)
