...
# Phase 253 — install Augment Auggie CLI. Called by livinityd cliInstaller.install
# tRPC mutation (whitelist-gated to SUPPORTED_CLIS).
#
# CRITICAL: the npm package is `@augmentcode/auggie` but the installed binary is
# `auggie` (NOT `augment`). Probe `command -v auggie`.
#
# Idempotency contract: if the `auggie` binary is already on PATH, log "already
...
step "Phase 253 — installing Augment Auggie CLI"
...
if command -v auggie >/dev/null 2>&1; then
    _v=$(auggie --version 2>/dev/null | head -1 || echo unknown)
    ok "✓ auggie already installed: ${_v}"
    exit 0
fi

if ! command -v npm >/dev/null 2>&1; then
    fail "augment: npm not on PATH — install Node.js first" 75
fi

info "Running: npm install -g --prefix ${NPM_PREFIX} @augmentcode/auggie"
if ! npm install -g --prefix "${NPM_PREFIX}" @augmentcode/auggie; then
    fail "augment: npm install failed" 75
fi
hash -r 2>/dev/null || true

if ! command -v auggie >/dev/null 2>&1; then
    fail "augment install completed but auggie binary still not on PATH" 75
fi
...
ok "augment (auggie) installed: $(auggie --version 2>/dev/null | head -1 || echo unknown)"
