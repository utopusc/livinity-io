...
# Phase 253 — install Qoder CLI. Called by livinityd cliInstaller.install
# tRPC mutation (whitelist-gated to SUPPORTED_CLIS).
#
# CRITICAL: the npm package is `@qoder-ai/qodercli` and the installed binary is
# `qodercli` (NOT `qoder`). Probe `command -v qodercli`. Auth is TUI
# slash-command wired in Plan 04.
#
# Idempotency contract: if the `qodercli` binary is already on PATH, log
# "already installed" and exit 0 without re-running npm install.
...
step "Phase 253 — installing Qoder CLI"
...
if command -v qodercli >/dev/null 2>&1; then
    _v=$(qodercli --version 2>/dev/null | head -1 || echo unknown)
    ok "✓ qodercli already installed: ${_v}"
    exit 0
fi

if ! command -v npm >/dev/null 2>&1; then
    fail "qoder-cli: npm not on PATH — install Node.js first" 75
fi

info "Running: npm install -g --prefix ${NPM_PREFIX} @qoder-ai/qodercli"
if ! npm install -g --prefix "${NPM_PREFIX}" @qoder-ai/qodercli; then
    fail "qoder-cli: npm install failed" 75
fi
hash -r 2>/dev/null || true

if ! command -v qodercli >/dev/null 2>&1; then
    fail "qoder-cli install completed but qodercli binary still not on PATH" 75
fi
...
ok "qoder-cli installed: $(qodercli --version 2>/dev/null | head -1 || echo unknown)"
