...
# Phase 253 — install Tencent CodeBuddy CLI. Called by livinityd cliInstaller.install
# tRPC mutation (whitelist-gated to SUPPORTED_CLIS).
#
# Package: `@tencent-ai/codebuddy-code` (binary `codebuddy`). Auth is TUI
# slash-command wired in Plan 04. Node 18.20+.
#
# Idempotency contract: if the `codebuddy` binary is already on PATH, log
...
step "Phase 253 — installing Tencent CodeBuddy CLI"
...
if command -v codebuddy >/dev/null 2>&1; then
    _v=$(codebuddy --version 2>/dev/null | head -1 || echo unknown)
    ok "✓ codebuddy already installed: ${_v}"
    exit 0
fi

if ! command -v npm >/dev/null 2>&1; then
    fail "codebuddy: npm not on PATH — install Node.js first" 75
fi

info "Running: npm install -g --prefix ${NPM_PREFIX} @tencent-ai/codebuddy-code"
if ! npm install -g --prefix "${NPM_PREFIX}" @tencent-ai/codebuddy-code; then
    fail "codebuddy: npm install failed" 75
fi
hash -r 2>/dev/null || true

if ! command -v codebuddy >/dev/null 2>&1; then
    fail "codebuddy install completed but binary still not on PATH" 75
fi
...
ok "codebuddy installed: $(codebuddy --version 2>/dev/null | head -1 || echo unknown)"
