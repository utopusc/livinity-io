...
//
// The original guard in mcp-client-manager.ts tested only the LITERAL hostname
// against a regex list. That is bypassed by:
...
//
// The private-range predicate is ported here (not imported) to keep liv/
// dependency-free of the livos webapps validator; it mirrors the same ranges as
// webapps/url-validator.ts isPrivateHost.
//
// Residual (T-257C-05, accepted): the transport may follow a redirect to a
// private host after this gate; pinning the resolved IP onto the live socket is
// transport-internal and is noted as a follow-up.
...
function octetsToString(octets: [number, number, number, number]): string {
  return octets.join('.');
}
...
export async function assertResolvedHostSafe(
  ...
  if (!ALLOWED_SCHEMES.has(parsed.protocol)) {
    throw new Error(`SSRF blocked: scheme '${parsed.protocol.replace(':', '')}' is not allowed (http/https only)`);
  }
  ...
  for (const addr of addresses) {
    if (isPrivateAddress(addr)) {
      throw new Error(`SSRF blocked: ${rawHost} resolves to a private/internal address (${addr})`);
    }
  }
}
